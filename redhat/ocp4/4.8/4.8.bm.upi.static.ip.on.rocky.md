...
wget https://raw.githubusercontent.com/wangzheng422/docker_env/dev/redhat/ocp4/4.7/scripts/helper-ks-rocky.cfg
...
ocp_client: "file:///data/ocp4/4.7.17/openshift-client-linux-4.7.17.tar.gz"
ocp_installer: "file:///data/ocp4/4.7.17/openshift-install-linux-4.7.17.tar.gz"
ppc64le: false
arch: 'x86_64'
chronyconfig:
  enabled: true
...
cd ${KVM_DIRECTORY}
scp root@172.21.6.11:/data/install/{*boot*,*master-0}.iso ${KVM_DIRECTORY}/
...
mkdir -p  ${KVM_DIRECTORY}
cd ${KVM_DIRECTORY}
scp root@172.21.6.11:/data/install/*master-1.iso ${KVM_DIRECTORY}/
...
mkdir -p  ${KVM_DIRECTORY}
cd ${KVM_DIRECTORY}
scp root@172.21.6.11:/data/install/{*worker-0,*master-2}.iso ${KVM_DIRECTORY}/
...
--disk path=/data/nvme/ocp4-bootstrap.qcow2,bus=virtio,size=50 \
--os-variant rhel8.4 --network bridge=baremetal,model=virtio \
...
export KVM_DIRECTORY=/data/kvm
virt-install --name=ocp4-master-0 --vcpus=20 --ram=36864 \
--cpu=host-model \
--disk path=/data/nvme/ocp4-master-0.qcow2,bus=virtio,size=120 \
--disk path=/data/nvme/ocp4-master-0-data-1.qcow2,bus=virtio,size=100 \
--disk path=/data/nvme/ocp4-master-0-data-2.qcow2,bus=virtio,size=100 \
--os-variant rhel8.4 --network bridge=baremetal,model=virtio \
...
# on kvm host 172.21.6.103
export KVM_DIRECTORY=/data/kvm
virt-install --name=ocp4-master-1 --vcpus=20 --ram=30720 \
--cpu=host-model \
--disk path=/data/kvm/ocp4-master-1.qcow2,bus=virtio,size=120 \
--disk path=/data/kvm/ocp4-master-1-data-1.qcow2,bus=virtio,size=100 \
--os-variant rhel8.4 --network bridge=baremetal,model=virtio \
...
# on kvm host 172.21.6.104
export KVM_DIRECTORY=/data/kvm
virt-install --name=ocp4-master-2 --vcpus=20 --ram=61440 \
--cpu=host-model \
--disk path=/data/kvm/ocp4-master-2.qcow2,bus=virtio,size=120 \
--disk path=/data/kvm/ocp4-master-2-data-1.qcow2,bus=virtio,size=100 \
--os-variant rhel8.4 --network bridge=baremetal,model=virtio \
...
# INFO Login to the console with user: "kubeadmin", and password: "Kp8Re-8yzte-IUEK8-Wmedx"
