...
打包好的安装包，在这里下载，百度盘下载链接，版本是 4.8.13 :
- 链接: https://pan.baidu.com/s/14sp0PW-qnoh93bGs3grkhA 提取码: 1mu4 
...
/bin/cp -f -Z container-local-registry.service  /usr/lib/systemd/system

systemctl enable --now container-local-registry.service
systemctl status container-local-registry.service

systemctl disable --now container-local-registry.service

systemctl daemon-reload
systemctl restart container-local-registry.service
...
platform:
  none: {}
pullSecret: '{"auths":{"registry.ocp4.redhat.ren:5443":{"username":"admin","password":"redhat","auth":"YWRtaW46cmVkaGF0","email":"admin@redhat.ren"}}}'
sshKey: |
...
cd ${KVM_DIRECTORY}
scp root@192.168.7.11:/data/install/{*boot*,*master-0,*worker-0,*worker-1}.iso ${KVM_DIRECTORY}/
...
virt-install --name=ocp4-worker-0 --vcpus=12 --ram=49152 \
...
virt-install --name=ocp4-worker-1 --vcpus=12 --ram=49152 \
...
# INFO Login to the console with user: "kubeadmin", and password: "Hdn4X-Ep8UR-s9fwM-xBJVq"
...
podman create --name swap quay.io/wangzheng422/qimgs:nexus-fs-image-2021-09-05-1553 ls
...
# 以下这个版本，可以作为初始化的image proxy，里面有基础的镜像源配置，但是没有镜像，在客户现场，一般需要配置一下http proxy
quay.io/wangzheng422/qimgs:nexus-fs-image-2021-09-05-1553
