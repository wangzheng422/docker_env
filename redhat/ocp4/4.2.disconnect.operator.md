...
- registry.with.operator.image.tgz  这个文件是docker image registry的仓库打包文件，需要复制到docker registry的服务器主机上去。由于包含了sample-operator，以及operator hub里面的很多image，所以比较大。sample-operator用到的镜像大小大概占一半大概40G。另外，由于4.2版本operator描述文件并不包含所有operator用到的image，所以里面的镜像并不全，大概是所有需要的image的1/3。作者会不断想办法补充，或者等到4.3，那个时候，描述文件会包含全部的image。
