...
打包好的安装包，在这里下载： https://drive.google.com/drive/u/2/folders/1B8cMGFp-MneySPvzg8wyarzIJ1ZEEgFf

其中包括4个文件：
- ocp4.tgz  这个文件包含了iso等安装介质，以及各种安装脚本，需要复制到宿主机，以及工具机上去。
- registry.with.operator.image.tgz  这个文件是docker image registry的仓库打包文件，需要复制到docker registry的服务器主机上去。由于包含了sample-operator，以及operator hub里面的很多image，所以比较大。sample-operator用到的镜像大小大概占一半大概40G。另外，由于4.2版本operator描述文件并不包含所有operator用到的image，所以里面的镜像并不全，大概是所有需要的image的2/3。作者会不断想办法补充，或者等到4.3，那个时候，描述文件会包含全部的image。
- registry.with.operator.hub.tgz  这个文件也是docker image registry的仓库打包文件，由于只有ocp和operator hub，以及少量的其他辅助镜像，所以比较小。但是要运行operator的话，需要先补充镜像。按照这里操作： https://github.com/wangzheng422/docker_env/blob/master/redhat/ocp4/4.2.add.image.md
- rhel-data.tgz 这个文件是 rhel 7 主机的yum更新源，这么大是因为里面有gpu, epel等其他的东西。 
- registry.?? 这些文件，是registry.with.operator.image.tgz的切分文件，按照10G大小切分，方便下载。 
...
```
Success
Update image:  registry.redhat.ren/ocp4/openshift4:4.2.7
...
spec:
  ...
  sourceType: grpc
  image: registry.redhat.ren/ocp-operator/custom-registry:community
EOF
