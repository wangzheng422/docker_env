...
打包好的安装包，在这里下载： https://drive.google.com/drive/u/2/folders/1uiw5sctkCYM-QdVYcZ7QQbQya84xS_NF

sha256sum:

e5451ec7115ebf02d243bc641d491aaf3f047db1aa82f21511e37ac6c17c97a7  ocp4.tgz

dbf88c2c4ab87a567c816e342ed1fd02152cd48a143c2f7a47b1c90b9833cfef  registry.tgz
...
# scp files/4.2/docker_image/* and files/4.2/operator_images/*

yum -y install jq python3-pip pigz
pip3 install yq
...
# 这一步也会运行很久，视网速而定，2-3个小时是要的。
bash image.mirror.sh

bash image.mirror.yaml.sh
# 上面的脚本有一个输出， image.mirror.yaml ，是重要的输出物。
...
```
Success
Update image:  registry.redhat.ren/ocp4/openshift4:4.2.0
...
INFO Login to the console with user: kubeadmin, password: 8jFdX-sBENb-4gujS-ojNiY
```
...
oc patch image.config.openshift.io/cluster -p '{"spec":{"additionalTrustedCA":{"name":"ca.for.registry"},"registrySources":{"insecureRegistries":["registry.redhat.ren"]}}}'  --type=merge
...
oc apply -f ./image.mirror.yaml

# 以下操作，删除mirror镜像信息，也会触发集群更新操作，请等待集群重启完毕
oc delete -f ./image.mirror.yaml

watch oc get node
```
从监控界面，能看到节点在升级，重启。未来会研究如何配置，避免升级。
...
# 如果想看到所有redhat, certified, community的operator，这样做
cat <<EOF > my-operator-catalog.yaml
...
spec:
  ...
  sourceType: grpc
  image: registry.redhat.ren/ocp-operator/custom-registry:all
EOF
oc create -f my-operator-catalog.yaml

# 如果想看到所有redhat, certified的operator，这样做
cat <<EOF > my-operator-catalog.yaml
apiVersion: operators.coreos.com/v1alpha1
kind: CatalogSource
metadata:
...
spec:
  displayName: My Operator Catalog
  sourceType: grpc
  image: registry.redhat.ren/ocp-operator/custom-registry:certified
EOF
oc create -f my-operator-catalog.yaml

# 如果想看到所有redhat的operator，这样做
cat <<EOF > my-operator-catalog.yaml
apiVersion: operators.coreos.com/v1alpha1
kind: CatalogSource
metadata:
...
spec:
  displayName: My Operator Catalog
  sourceType: grpc
  image: registry.redhat.ren/ocp-operator/custom-registry:redhat
EOF
oc create -f my-operator-catalog.yaml

# 想删除这个离线operator hub，就这样做。
oc delete -f my-operator-catalog.yaml
