...
#### 2. Switch to NFSv3 for Better Locking Stability

NFSv4 uses a stateful locking mechanism that can sometimes cause issues like "lost lock" in high-concurrency environments like virtualization. NFSv3, on the other hand, uses a stateless locking model that is often more robust for these workloads. Switching the client to use NFSv3 is a common and effective solution.

I have updated the `StorageClass` definition to use `nfsvers=3`.

#### 3. Optimize /etc/exports
