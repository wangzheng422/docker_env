...
# /etc/ganesha/ganesha.conf - Minimal configuration example
...
# NFS CORE PARAMETERS
NFS_CORE_PARAM {
    # Enforce NFSv4 protocol only, which is more modern and secure.
    Nfs_Protocols = 4;
}
...
EXPORT {
    ...
    # The local physical path on the server to be exported.
    Path = "/srv/nfs/openshift-01";

    # The path clients see when mounting (the pseudo filesystem path for NFSv4).
    Pseudo = "/openshift-01";

    # Access permissions.
    Access_Type = RW; # Read-Write
    Squash = No_Root_Squash; # Allows the client's root user to have root access.

    # Security protocol.
    SecType = "sys";

    # Define the backend storage type (FSAL - File System Abstraction Layer).
    FSAL {
        Name = XFS; # Use XFS as the backend.
    }

    # Client access control.
    CLIENT {
        Clients = "0.0.0.0/0"; # Allows all clients. For production, change to specific IPs or subnets.
        Access_Type = RW;
    }
}
...
3.  **验证导出是否成功**：
    使用`showmount`命令查看本地NFS服务器的导出列表。
    ```bash
    showmount -e localhost
    ```
    您应该能看到类似下面的输出，显示了我们配置的伪路径：
    ```
    Export list for localhost:
    /ganesha-share 0.0.0.0/0
    ```
...
sudo mount -t nfs -o nfsvers=4.2 <ganesha-server-ip>:/ganesha_share /mnt
```
