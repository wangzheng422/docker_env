...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-01,device=disk,bus=virtio,format=raw,discard=unmap,cache=default \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_01_INTERFACE_MAC,driver_queues=$SNO_CPU \
--network bridge=br-ocp,model=virtio,mac=$MASTER_01_INTERFACE_02_MAC,driver_queues=$SNO_CPU \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-02,device=disk,bus=virtio,format=raw,discard=unmap,cache=default \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_02_INTERFACE_MAC,driver_queues=$SNO_CPU \
--network bridge=br-ocp,model=virtio,mac=$MASTER_02_INTERFACE_02_MAC,driver_queues=$SNO_CPU \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-03,device=disk,bus=virtio,format=raw,discard=unmap,cache=default \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_03_INTERFACE_MAC,driver_queues=$SNO_CPU \
--network bridge=br-ocp,model=virtio,mac=$MASTER_03_INTERFACE_02_MAC,driver_queues=$SNO_CPU \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-01,device=disk,bus=virtio,format=raw,discard=unmap,cache=default \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_01_INTERFACE_MAC,driver_queues=$SNO_CPU \
--network bridge=br-ocp,model=virtio,mac=$MASTER_01_INTERFACE_02_MAC,driver_queues=$SNO_CPU \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-02,device=disk,bus=virtio,format=raw,discard=unmap,cache=default \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_02_INTERFACE_MAC,driver_queues=$SNO_CPU \
--network bridge=br-ocp,model=virtio,mac=$MASTER_02_INTERFACE_02_MAC,driver_queues=$SNO_CPU \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-03,device=disk,bus=virtio,format=raw,discard=unmap,cache=default \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_03_INTERFACE_MAC,driver_queues=$SNO_CPU \
--network bridge=br-ocp,model=virtio,mac=$MASTER_03_INTERFACE_02_MAC,driver_queues=$SNO_CPU \
...
vers4.2=y
vers3=n  # 禁用 v3
lease-time=120  # 默认是 90 秒，可以适当增加
grace-time=120  # 宽限时间也相应增加

[lockd]
# Set the lockd grace period (in seconds)
grace-time=120
```
...
sudo systemctl status nfs-server rpc-statd nfs-idmapd
```
...
3.  **创建并设置要导出的目录**：
    ```bash
    # 创建目录
    sudo mkdir -p /srv/ganesha-share

    # 设置一个宽松的权限以便测试，生产环境请根据需求调整
    sudo chmod 777 /srv/ganesha-share
    sudo chown nobody:nobody /srv/ganesha-share
    ```
...
2.  **配置SELinux**:
    ...
    sudo semanage fcontext -a -t nfsd_file_t "/srv/ganesha-share(/.*)?"
    sudo restorecon -Rv /srv/ganesha-share
    ```
