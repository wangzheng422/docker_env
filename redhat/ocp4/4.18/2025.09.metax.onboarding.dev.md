...
### install with gpu metrics export

```bash

# get the container image, and push to internal image registry
...
enable openshift workload monitoring
...
For 2025.12.12, there are new version of metax k8s gpu operator, we try it out.
...
# cr.metax-tech.com/cloud/mx-exporter:0.13.2
...
# there are bugs in the official kernel driver image, 
# we need to merge internal and offical kernel driver image together.
# build the driver image again
tee driver-image.dockerfile << EOF
...
# push the driver image to local registry
...
# push the driver image to quay.io
...
# there is bugs in metaxk k8s operator, runtime, maca components does not work
# so we manually label the node
# so the driver deamonset will work
# re-apply the label if ocp node restart.
...
# this is the changes made by metax runtime
cat /etc/crio/crio.conf.d/88-metax.conf
# [crio]

#   [crio.runtime]
#     default_runtime = "metax"

#     [crio.runtime.runtimes]

#       [crio.runtime.runtimes.metax]
...
#         no_sync_log = false
#         runtime_config_path = ""
#         runtime_path = "/var/lib/metax/tools/usr/bin/mx-container-runtime"
#         runtime_root = "/run/crun"
#         runtime_type = "oci"

#         [crio.runtime.runtimes.metax.default_annotations]
#           "run.oci.systemd.subgroup" = ""
```

test the env with demo pod.
```yaml
# Save as gpu-demo.yaml
apiVersion: v1
kind: Pod
...
spec:
  ...
  containers:
  - name: vector-add
    image: mirror.infra.wzhlab.top:8443/metax/maca-native:3.3.0.4-centos9-amd64
    command: [
        "bash",
        "-c",
        "cp -r /opt/maca/samples/0_Introduction/vectorAdd /home;
...
For openshift coreos, it is based on rhel 9.6, and metaxk kernel driver can not run without default parameter, so we need to build a DIY driver loader, to load the kernel driver.

Here is the driver loader's image.
```bash

tee driver-init.dockerfile << EOF
...
And here is the driver loader's script.
```yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
And here is the driver loader's daemonset.
```yaml
apiVersion: apps/v1
kind: DaemonSet
metadata:
...
To run very large llm with vllm, like Qwen3-235B-A22B-Instruct-2507, you have to apply some patch to ocp node, because currently, vllm use ray to run multi-node interencing, and it require very high pid, memory limit. 

We need such crio config plugin"
```ini
[crio.runtime]
pids_limit = 0
default_ulimits = [
...
So we apply these machine config to openshift cluster.
```yaml
apiVersion: machineconfiguration.openshift.io/v1
kind: MachineConfig
metadata:
  labels:
    machineconfiguration.openshift.io/role: worker # 这一行指定应用到 worker 节点池
...
spec:
  config:
    ...
    storage:
      files:
        - contents:
            # 下面的 source 是经过 URL 编码的 TOML 配置内容
            # 原文内容见下方注释
...
And change kubeletconfig, so cgroup will not limit the pids.
...
spec:
  # 1. 选择要应用此配置的节点池 (MachineConfigPool)
  # 通常 AI 节点都在 worker 池，或者你有专门的 gpu-worker 池
  ...
  # 2. Kubelet 参数配置
  kubeletConfig:
    # 设置为 -1 表示使用宿主机内核的最大限制 (即 pid_max)
    # 这相当于 podman 的 --pids-limit=-1
    # 如果不想完全放开，也可以设为一个很大的整数，例如 4194304
    podPidsLimit: -1
```

### run with ocp-ai inference service

here is the example configuration, if you want to run interence service with relative small LLM, like qwen-32b

First, it is the servingruntime
```yaml
...
metadata:
  annotations:
    ...
    opendatahub.io/runtime-version: v1.0.1
    openshift.io/display-name: vLLM MetaX wzh testing
    opendatahub.io/apiProtocol: REST
  labels:
    opendatahub.io/dashboard: "true"
  name: vllm-maca-runtime-wzh-testing
spec:
  annotations:
...
And here is the inference service
...
Here is another example of serving runtime
...
metadata:
  annotations:
    opendatahub.io/recommended-accelerators: '["metax-tech.com/gpu"]'
    opendatahub.io/runtime-version: v1.0.0
    openshift.io/display-name: Bash of vLLM MetaX wzh testing
    opendatahub.io/apiProtocol: REST
  labels:
    opendatahub.io/dashboard: "true"
  name: vllm-maca-runtime-wzh-testing-bash
spec:
  annotations:
    opendatahub.io/kserve-runtime: vllm
    prometheus.io/path: /metrics
...
metadata:
  annotations:
    openshift.io/display-name: wzh-test-bash
    serving.kserve.io/deploymentMode: RawDeployment
    serving.kserve.io/stop: 'true'
  name: wzh-test-bash
  namespace: models
    ...
    model:
      ...
      resources:
        ...
        requests:
          ...
          memory: 16Gi
          metax-tech.com/gpu: '2'
      runtime: wzh-test-bash
      storageUri: 'oci://quay.io/jonkey/models/qwen3:32b'

```

### run with leader workset operator

to run vllm interencing on 2 node, with 8 metax gpu each, you can use leader workset, here is an example config.

The keypoint in this config, is to use hostnetwork, as ray control data channel go through ovn, the performance will degrade at least 500ms for FTTT.
