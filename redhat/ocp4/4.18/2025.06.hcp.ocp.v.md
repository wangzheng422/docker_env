...
The most important aspect is we will analyze how the ingress network traffic path looks like.
...
We need storage to provide PVCs and VM disks. For simplity, we will use LVM, it is local disk/lvm, so no vm hot migration support.
...
We do not need ACM; we only need MCE for HCP over ocp-v. The MCE is slice of ACM.
...
After enable MCE, wait around 10 mins, it will install some pods, and after it finished, you can see the new webUI.
...
Okay, let's start with HCP. First, we need patch the ingress, so it will pass all hosted cluster ingress traffic from host cluster ingress into the hosted cluster.
...
Next, verify your dns settings, so the hosted cluster's domain name can be resolved. Set DNS record
- `*.apps.demo-01-rhsys.wzhlab.top` -> `192.168.35.22`
  - This means: `*.apps.wzh-01.demo-01-hcp.wzhlab.top` -> `192.168.35.22`

## using webUI

Ok, lets create the hosted cluster.
...
> [!NOTE]
> The auto generated CIRD will conflict with host cluster
...
After create the hosted cluster, we just need to wait, it will create control plan pods, and create ocp-v vm as worker node.
...
Login to the hosted cluster, you can see we got a new ocp cluster.
...
First, you need the cli tool on local host.
...
Then, you create the hosted cluster with parameters.
