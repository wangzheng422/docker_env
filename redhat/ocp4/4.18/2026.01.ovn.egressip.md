...
graph TD
    subgraph OCP Cluster
        subgraph ns-blue Business Namespace
            BP[Business Pods<br/>*Matched by Controller*]
            APB[APB Route<br/>Policy Routing]
        end
        
        subgraph ns-egress-infra Infrastructure Namespace
            GP[Gateway Pod<br/><b>Multi-Network</b><br/>eth0: OVN Network<br/>macvlan0: Physical Network]
            CTRL[APB Controller]
        end
    end
    ...
    %% Traffic Flow
    BP -- "1. Traffic to External" --> APB
    APB -- "2. Redirect via OVN (eth0)" --> GP
    GP -- "3. SNAT & Egress <b>via macvlan0</b>" --> EXT
    
    %% Control Plane & OVN Patches
    CTRL -. "Watch IP" .-> GP
    CTRL -. "Update NextHop" .-> APB
    CTRL -. "<b>OVN Patch 1</b><br/>Clear Port Security<br/>Allow All Stateless ACL" .-> GP
    CTRL -. "<b>OVN Patch 2</b><br/>Allow Return Traffic<br/>Stateless ACL" .-> BP
    
    %% Styling
    style BP fill:#e1f5fe,stroke:#01579b
    style GP fill:#e8f5e9,stroke:#2e7d32
    style APB fill:#fff3e0,stroke:#ef6c00
