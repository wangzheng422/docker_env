...
# 1. Delete old local rolebinding (cleanup)
...
# 3. Create ClusterRoleBinding (Bind the SA in ns-egress-infra to the ClusterRole)
cat <<EOF | oc apply -f -
apiVersion: rbac.authorization.k8s.io/v1
...
# Grant privileged SCC to the SA
    ...
    spec:
      ...
      - name: gateway
        ...
        - |
          ...
          # Create separate route table for traffic coming from OVN network
          ...
          # Tested: The above routes do not work
          ip route del default
          ip route add default via 192.168.99.1 dev macvlan0
...
1.  **Isolate Gateway Namespace**
    ```bash
    # Use a dedicated infra namespace
    # Restrict who can create pods in this namespace
    oc adm policy add-role-to-user admin user1 -n ns-egress-infra
    ```

2.  **Restrict Business Namespace**
    ```bash
    # Use ResourceQuota to limit pod count
    # Avoid having too many pods bypassing Network Policy
    ```

3.  **Monitor and Audit**
    ```bash
    # Periodically check ACL configurations
...
Please refer to the Controller Deployment section below for details.
...
```bash
# Corresponding Gateway Deployment
cat <<'EOF' | oc apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
