...
pv metat data, please note that the pv point to nfs server directory on `/openshift-01`.
...
pay attention to the pv name, as the logic in oadp plugin, it contact the name of pvc and snapshot.
...
### check the log of oadp plugin

check the restore plugin log, you can see new pvc is restored from snapshot
...
pvc meta data. Please note that the pvc is point to the same name of pv as main site, it will not re-create a new pv on DR site.
...
pv metat data. Please note that the pv is patched, and  point to nfs server directory on `/openshift-02`, which is the logic of our customer oadp plugin.
...
check the restore plugin log, you can see the pv is patched.

```bash
oc logs -n openshift-adp -l app.kubernetes.io/name=velero --tail=-1 | grep modify-pv-handle.go
Defaulted container "velero" out of: velero, openshift-velero-plugin (init), kubevirt-velero-plugin (init), velero-plugin-for-aws (init), csi-volume-dr (init), snapshot-dr (init)
...
1.  mainsite cleanup: on mainsite, if backup perform, a new pvc is created for each snapshot, if the snapshot is removed, and the restored pvc is not referenced by any vm, the restored pvc should be deleted.
2.  check the backup result, because the oadp plugin logic, it will failed on some snapshot, if the restored pvc is found already created, so the backup result should be partially failed. So ansible/automation should check the backup result, make sure no other part of the backup is failed.
...
4.  cleanup pvc after failover/restore: oadp restore will only update and add pvc,  if pvc is removed on mainsite, it should be remove on DR site also.
5.  remove pv on both site, because we use `retain` policy.
