...
## gpu operator (helm based)

metax support runs a gpu operator to dynamic inject kernel driver to rhcos. Let's try it out.

Supported ocp version and corresponding kernel version right now.
...
First, get the image files, download it, and upload to internel registry
...
Then, get the helm chart and install it
...
If you want to uninstall the helm chart
...
After helm chart install, an operator pod start, and will trigger some daemon set. Currently, no runtime supported, so we will patch the daemon set to work as we expected.
