...
--set driver.payload.name=driver-image \
--set driver.payload.version=3.3.0.4-amd64


# create sa for service account metax-operator/metax-gpu-label, and give it priviledges
oc create sa metax-gpu-label -n metax-operator
oc adm policy add-scc-to-user privileged -z metax-gpu-label -n metax-operator
oc patch daemonset metax-gpu-label -n metax-operator --type='merge' -p '{"spec":{"template":{"spec":{"serviceAccountName":"metax-gpu-label"}}}}'
