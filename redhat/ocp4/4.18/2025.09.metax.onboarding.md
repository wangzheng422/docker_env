...
The standard RHCOS image does not contain the necessary drivers for Metax GPUs. Therefore, we must build a custom image. This process involves setting up a build environment, preparing RPM repositories, and using the `coreos-assembler` (cosa) tool.
...
# Download the specific kernel version required by RHCOS 4.18 and its dependencies
PACKAGES_TO_QUERY=(
