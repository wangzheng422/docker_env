...
# on ocp-02
# give admin to edit status
cat << EOF > $BASE_DIR/data/install/status-editor.yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata:
  name: status-editor
rules:
- apiGroups: ["snapshot.storage.k.io"]
  resources:
    # 允许对VolumeSnapshot对象本身进行常规操作
    - volumesnapshots
  verbs: ["get", "list", "watch", "update", "patch"]
- apiGroups: ["snapshot.storage.k.io"]
  resources:
    # 关键：明确允许对/status子资源进行更新和修补
    - volumesnapshots/status
  verbs: ["update", "patch"]
EOF
oc apply -f $BASE_DIR/data/install/status-editor.yaml

oc create clusterrolebinding my-status-editor-binding --clusterrole=status-editor --user=admin
...
spec:
  ...
  volumeSnapshotRef:
    ...
    name: vmsnapshot-f8187894-9cd0-48cf-8046-1d72b59c7780-volume-rootdisk
    namespace: demo
    uid: 286c25dd-efb2-4f7e-b95a-4f20385c752f # get uid after above object created
...
path to virtualMachineSnapshotContent
