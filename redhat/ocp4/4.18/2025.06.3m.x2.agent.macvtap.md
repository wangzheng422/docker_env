...
# 这不会影响您已有的br-ocp或internal-net网络
...
# create a new address for helper to communicate with macvtap VMs
# ip link add link eth0 name eth0.99 type macvtap mode bridge
# ip addr add 192.168.99.10/24 dev eth0.99
# ip link set eth0.99 up

# sudo nmcli connection add \
#   type macvlan \
#   con-name "host-kvm-link" \
#   dev eth0 \
#   ifname macvtap0 \
#   mode bridge \
...
# host-kvm-link  51df2a66-1f9b-4557-a1b9-c8df796e8537  macvlan   macvtap0
# lo             d63cb460-11f6-46ab-a8b3-1860084f96ff  loopback  lo

# 创建一个无IP的ipvlan接口作为上行链路
sudo nmcli connection add \
  type ipvlan \
  con-name "uplink-for-kvm-bridge" \
  dev eth0 \
  ifname ipvlan0 \
  mode l2 \
...
# 2. 将ipvlan0接口作为“奴隶”添加到网桥中
sudo nmcli connection add \
  type bridge-slave \
  con-name "br-slave-ipvlan0" \
  ifname ipvlan0 \
  master br-kvm

# 3. 激活网桥（它会自动激活它的奴隶接口ipvlan0）
sudo nmcli connection up "kvm-bridge"


# 允许从br-kvm发往eth0的新建连接（虚拟机访问外网）
sudo iptables -A FORWARD -i br-kvm -o eth0 -j ACCEPT

# 允许从eth0返回到br-kvm的、已经是已建立连接的流量
sudo iptables -A FORWARD -i eth0 -o br-kvm -m state --state RELATED,ESTABLISHED -j ACCEPT


# --- 请根据您的实际网络配置修改这两个变量 ---
KVM_SUBNET="192.168.99.0/24"
VPC_SUBNET="192.168.99.0/24" # 这是您VPC的网段

# 添加新的规则
sudo iptables -t nat -A POSTROUTING -s "${KVM_SUBNET}" ! -d "${VPC_SUBNET}" -j MASQUERADE
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-01,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
--os-variant rhel9.3 \
--network bridge=br-kvm,source_mode=bridge,model=virtio,mac=$MASTER_01_INTERFACE_MAC \
--graphics vnc,listen=127.0.0.1,port=59001 --noautoconsole \
--boot menu=on --cdrom /data/kvm/agent.x86_64.iso

# --iothreads=$(($SNO_CPU/3)) \
# ,driver_queues=$SNO_CPU
# --disk path=/dev/vgdata/lv-demo-01-master-01-data,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
# --network type=direct,source=eth0,source_mode=bridge,model=virtio,mac=$MASTER_01_INTERFACE_02_MAC \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-02,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
--os-variant rhel9.3 \
--network bridge=br-kvm,source_mode=bridge,model=virtio,mac=$MASTER_02_INTERFACE_MAC \
--graphics vnc,listen=127.0.0.1,port=59002 --noautoconsole \
--boot menu=on --cdrom /data/kvm/agent.x86_64.iso

# --disk path=/dev/vgdata/lv-demo-01-master-02-data,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
# --network type=direct,source=eth0,source_mode=bridge,model=virtio,mac=$MASTER_02_INTERFACE_02_MAC \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-03,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
--os-variant rhel9.3 \
--network bridge=br-kvm,source_mode=bridge,model=virtio,mac=$MASTER_03_INTERFACE_MAC \
--graphics vnc,listen=127.0.0.1,port=59003 --noautoconsole \
--boot menu=on --cdrom /data/kvm/agent.x86_64.iso

# --disk path=/dev/vgdata/lv-demo-01-master-03-data,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
# --network type=direct,source=eth0,source_mode=bridge,model=virtio,mac=$MASTER_03_INTERFACE_02_MAC \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-01,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
--os-variant rhel9.3 \
--network network=macvtap-net,model=virtio,mac=$MASTER_01_INTERFACE_MAC \
--network network=macvtap-net,model=virtio,mac=$MASTER_01_INTERFACE_02_MAC \
--graphics vnc,listen=127.0.0.1,port=59011 --noautoconsole \
--boot menu=on --cdrom /data/kvm/agent-02.x86_64.iso

# --disk path=/dev/vgdata/lv-demo-02-master-01-data,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-02,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
--os-variant rhel9.3 \
--network network=macvtap-net,model=virtio,mac=$MASTER_02_INTERFACE_MAC \
--network network=macvtap-net,model=virtio,mac=$MASTER_02_INTERFACE_02_MAC \
--graphics vnc,listen=127.0.0.1,port=59012 --noautoconsole \
--boot menu=on --cdrom /data/kvm/agent-02.x86_64.iso

# --disk path=/dev/vgdata/lv-demo-02-master-02-data,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-03,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
--os-variant rhel9.3 \
--network network=macvtap-net,model=virtio,mac=$MASTER_03_INTERFACE_MAC \
--network network=macvtap-net,model=virtio,mac=$MASTER_03_INTERFACE_02_MAC \
--graphics vnc,listen=127.0.0.1,port=59013 --noautoconsole \
--boot menu=on --cdrom /data/kvm/agent-02.x86_64.iso

# --disk path=/dev/vgdata/lv-demo-02-master-03-data,device=disk,bus=virtio,format=raw,discard=unmap,cache=none \
...
# /etc/ganesha/ganesha.conf - Corrected Configuration
...
NFS_CORE_PARAM {
    Nfs_Protocols = 4;
    # SecType is configured per-export, not globally.
}

# EXPORT BLOCK - Defines a share for openshift-01
EXPORT {
    Export_Id = 10;
    Path = "/srv/nfs/openshift-01";
    Pseudo = "/openshift-01";
    Access_Type = RW;
    Squash = No_Root_Squash;
    SecType = "sys"; # Security type defined here
    FSAL {
        Name = VFS;
    }
}
...
```
    您应该看到 `Active: active (running)`。如果有错误，可以查看日志 `sudo journalctl -u nfs-ganesha.service` 或 `/var/log/ganesha.log` 来排查问题。
...
sudo mount -t nfs -o nfsvers=4.2 192.168.99.1:/srv/nfs/openshift-01 ./mnt
```
...
mc alias set myminio http://192.168.99.10:9000 minioadmin minioredhat
