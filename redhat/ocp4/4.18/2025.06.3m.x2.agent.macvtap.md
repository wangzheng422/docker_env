...
# 循环从 0 到 (NUM_INTERFACES - 1)
for (( i=0; i<${NUM_INTERFACES}; i++ )); do
  
  # 格式化Master节点的编号 (例如: 1 -> 01, 2 -> 02)
  MASTER_NUM=$(printf "%02d" $((i + 1)))
  
  # 定义连接名称和接口名称
  CONN_NAME="ipvlan-for-master-${MASTER_NUM}"
  IFACE_NAME="ipvlan${i}"
...
--os-variant rhel9.3 \
--network bridge=ipvlan0,source_mode=bridge,model=virtio,mac=$MASTER_01_INTERFACE_MAC \
...
--os-variant rhel9.3 \
--network bridge=ipvlan1,source_mode=bridge,model=virtio,mac=$MASTER_02_INTERFACE_MAC \
...
--os-variant rhel9.3 \
--network bridge=ipvlan2,source_mode=bridge,model=virtio,mac=$MASTER_03_INTERFACE_MAC \
