...
First, we try to reproduce what will be wrong. Let create a new folder on nfs server, this is the target nfs folder we will test for.
...
Next, we will create pv, pvc, and pod running on cluster-01, to write to the nfs folder.
...
We can see, the pod is running in uid `1000800000`, and the file is written.

Now check on nfs server
...
Next, we will create pv, pvc, and pod running on cluster-02, to write to the nfs folder.
...
We can see, the pod is running in uid `1000790000`, and the file is not written.
...
We have seen what the problem is, now we try to fix it on ocp.

First, we create another folder on nfs server, this time, we will create a folder with 严格的 permission.
...
Next, on cluster-01, create pv, pvc, pod again.
...
We can see the pod is running in uid `1001`, and the file is written.
...
We can see the file on nfs with different uid `1001` and gid `2002`.

## on cluster-02

Next, on cluster-02, create pv, pvc, pod again.
...
We can see, this time on DR site, the write is ok.

## mount on rhel node

We already know how to fix the issue on ocp, but what about if the nfs folder is shared with a rhel node?

Let us test this case on rhel node.
...
Let try with another user in the same group.
