...
Continue with previous [metalLB on 2nic testing](./2025.07.metallb.2nic.md), we will try to explore expose host control plan's hosted cluster throught 2nd network.
...
metalLB setting part is the same with preview doc, you can reference it at [here](./2025.07.metallb.2nic.md#enable-ip-forward-for-nodes).
...
Because we have a customized nfs storage, we need to let cnv recogonize this nfs storage.
...
The hosted cluster can not finish, because the ingress compnent can not start, so we need to make some configuration change on metalLB to fix it.
...
Wait a moment, the hosted cluster will be ready. We are curious about how the ingress traffic path is, here is the cli output. It hits iptables again, and forward into ovn, and there is a rule/policy in ovn to rorward it to vm.
...
The hosted cluster only have `kubeadmin` user at the beginning, we give it `htpasswd` auth method to access the cluster.
...
At the end, lets go with web console, to test it. we are using a disconnected vm, only connect to 2nd network, so we use `/etc/hosts` to provide domain name resolve.
...
Then we go to the vm console, and using the browser to access the hosted cluster webconsole.

<img src="imgs/2025.06.hcp.ocp.v.metallb.2nic.md/2025-07-23-16-15-38.png" width="1024">

We can check the hosted cluster's status on MCE console too.
