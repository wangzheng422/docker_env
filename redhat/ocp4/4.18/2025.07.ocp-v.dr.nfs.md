...
spec:
  ...
  # 关键字段：在这里设置资源冲突策略
  # 可选值: 'update' 或 'none'
  existingResourcePolicy: 'update' # <-- 设置为 'Update' 来覆盖已存在的资源 
EOF
...
spec:
  ...
  driver: nfs.csi.k8s.io
  source:
    snapshotHandle: 192.168.99.1#srv/nfs/openshift-02#snapshot-a3ad8ba8-4ba7-4a67-85b3-5517a1d4c06c#snapshot-a3ad8ba8-4ba7-4a67-85b3-5517a1d4c06c#pvc-430cb55c-9e9f-4eb4-ad44-0b797d0c5a56 # 确保指向nfs02
...
Now, do oadp restore without pv. And you can see vm take another snapshot, not using the one we sync from nfs-01.

Now, skip the virtualMachineSnapshotContent, and virtualMachineSnapshot, and try to patch manually

get uid of vm `c789fb62-c239-45a7-938c-41225eb43e29`.

update virtualMachineSnapshotContent with this vm uid, and remove any other id and reversion, label, and annotations.
...
snapshot restored, but with some minor issues, like snapshot volumes
...
and patch `VirtualMachineSnapshotContent`
...
Then the `virtualMachineSnapshot` back to normal
