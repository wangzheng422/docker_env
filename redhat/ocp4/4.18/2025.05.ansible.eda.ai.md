...
There are offical ways to make webhook call from service.now to aap, but it can not be activeated in the developer version of ServiceNow.
...
## using rulebook to trigger create service.now incident and change request

Now, we test the EDA senario, we will create a rulebook, and trigger a workflow to create something in service.now.

![](./files/aap.snow.03.drawio.png)

setting aap credentials for the rulebook in AAP, so it can trigger workflow.
...
In out proposed solution, we will store the GenAI created playbook in git repo with new branch, after approve, merge it back, and run it. Now we will test this part. We will run the playbook directly in ansibe job instance, and store the log in the git repo too. Another approch is create a new ansible job template using the GenAI playbook, and launch a new job for the created job teamplate, we will test this approach later.
...
The job template needs `extra var`, and credential with `prompt on launch`

<img src="imgs/2025.05.ansible.eda.ai.md/2025-06-26-10-03-47.png" width="1024px">

And define a workflow template.
...
We will call deepseek(througth openai API) to generate a new ansible playbook to fix the problem.
