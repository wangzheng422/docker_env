# Implementing Layer 4 Switch as Ingress Gateway for OpenShift: Challenges and Mitigation Strategies

## 1. Executive Summary and Architectural Context

In enterprise-grade OpenShift deployments, the industry standard best practice is to utilize a Layer 7 (L7) Load Balancer positioned in front of the OpenShift Router (Ingress Controller). L7 Load Balancers possess the capability to inspect application-layer data, enabling them to query specific health verification endpoints (e.g., `HTTP GET /healthz/ready`). This granular inspection ensures that traffic is distributed exclusively to router pods that are fully initialized and operationally ready to process requests.

However, physical infrastructure constraints or legacy networking policies often necessitate the use of Layer 4 (L4) switches. Unlike their L7 counterparts, L4 switches operate strictly at the transport layer, typically relying on rudimentary TCP connection attempts (SYN/ACK handshakes) to determine backend availability. This limitation introduces a critical **race condition** during the lifecycle events of router pods, such as auto-scaling, restarts, or rolling upgrades:

1.  **Premature Traffic Routing:** When a new Router Pod initializes, the container's network stack and the underlying operating system immediately open the listening ports (TCP 80/443).
2.  **False Positive Health Check:** The L4 switch detects the open TCP port and immediately classifies the backend node as "UP".
3.  **Service Unavailability:** The application layer (HAProxy) within the pod may still be in its startup phase—loading SSL certificates, parsing large configuration maps—and is not yet ready to serve traffic.
4.  **Request Failures:** Client traffic routed to this "zombie" pod results in connection resets or HTTP 503 Service Unavailable errors, causing distinct service interruptions.

To overcome this limitation without necessitating costly infrastructure upgrades to L7 hardware, we present a robust architectural pattern: the **"Sidecar Health Check Translator"**. This solution introduces a lightweight daemon that bridges the gap between the Router's internal L7 application status and the L4 switch's TCP-based expectations.

## 2. Technical Architecture: The Sidecar Translator Pattern

The proposed solution involves deploying a specialized daemon (via a DaemonSet) on every node that hosts an OpenShift Router. This "Sidecar" agent acts as a proxy for health status, translating application-layer readiness into transport-layer connectivity.

### Functional Logic

1.  **Monitor (Layer 7):** The daemon continuously polls the local Router's application health endpoint (`http://localhost:1936/healthz/ready`) via the loopback interface.
2.  **Translate (Business Logic):**
    *   **Healthy State:** If the router returns `200 OK`, the daemon opens a dedicated TCP port (e.g., `18898`) on the host network.
    *   **Unhealthy State:** If the router returns any error code or times out, the daemon actively closes or refuses connections on port `18898`.
3.  **Route (Layer 4):** The external L4 switch is configured to perform its TCP health check against this **Translator Port (`18898`)** instead of the actual traffic ports (`80`/`443`). Traffic is only forwarded to the node when the Translator Port is open, which now accurately confirms that the underlying Router application is fully operational.
...
## 3. Simulation and Validation Environment

To rigorously validate this solution, we will simulate the production environment using a local HAProxy instance acting as the external Load Balancer. The verification process is conducted in three distinct phases:

1.  **Phase 1 (Baseline):** Establishing the ideal behavior using native L7 health checks.
2.  **Phase 2 (Problem Reproduction):** Demonstrating the failure mode when degraded to standard L4 checks.
3.  **Phase 3 (Solution Verification):** Proving the efficacy and reliability of the L4 Health Check Translator.

**Demo Deployment Architecture:**

![](./files/l4.swtich.01.drawio.png)

### 3.1 Phase 1: Establishing the Baseline with Layer 7 Health Checks

Initially, we install HAProxy to function as our load balancer simulator.
...
We must verify that SELinux policies allow HAProxy to establish connections to any port, which is necessary for our simulation setup.
...
We configure HAProxy as a **Layer 7 Load Balancer**.
**Key Configuration Details:**
*   `mode http`: Enables L7 protocol inspection.
*   `option httpchk`: Performs an active HTTP GET request to `/healthz/ready` to determine backend health.
*   `check port 1936`: Directs the health check to the router's dedicated status port (1936), independent of the ingress traffic port (80).
...
To generate consistent synthetic load, we utilize `hey`, a modern and efficient HTTP load testing utility.
...
**Test Execution:**
We initiate the load generator. During the execution of the test, we will manually delete a router pod. Since L7 health checks are active, HAProxy should instantly detect the failure via the `1936` port and cease routing traffic to that specific pod, ensuring zero dropped requests.
...
**Result Analysis:**
The test logs demonstrate a **100% success rate (200 OK)**. This confirms that intelligent L7 health checks correctly handle pod churn by validating the application state before routing traffic.
...
### 3.2 Phase 2: Reproducing the Race Condition (L4 Failure Mode)

Now, we intentionally degrade the simulation to mimic a standard L4 switch environment. We disable the HTTP health checks and configure HAProxy to rely solely on basic TCP port availability.
...
**Test Execution:** We repeat the exact same load test and fault injection scenario.
...
**Result Analysis:**
We observe **6 failed requests (503 errors)**. This downtime corresponds to the critical "race condition" window where the new router pod's operating system had opened the port, but the HAProxy application was not yet ready to serve requests. The L4 switch, seeing the open port, incorrectly routed traffic into a "black hole".
...
We now deploy the Health Check Translator to mitigate the issue identified in Phase 2.

**Step 1: Deploy the Logic Script**
We create a `ConfigMap` containing the Python logic. This script uses standard library modules to check the router and manage a TCP listener process. It serves as the intelligent "bridge".
...
We deploy the script as a DaemonSet to ensure one agent runs on every node.
**Key Configuration Details:**
*   `hostNetwork: true`: **Crucial.** This allows the pod to bind the port `18898` directly on the host node's IP interface, making it accessible to the external physical network/load balancer.
*   `privileged: true`: Required for host network operations and potentially accessing local loopback services depending on the cluster's security policies.
...
We finally reconfigure HAProxy to use the **Translator Port (18898)** for health checks. This tells the L4 switch to ask our Sidecar Agent "Are you ready?" instead of blindly trusting the port open status of the router.
...
**Test Execution:** We perform one final run of the load test with the full solution in place.
...
**Result Analysis:**
The results confirm the fix. We achieve a **100% success rate**, effectively bringing L7-like resilience to L4 infrastructure.
...
## 4. Reliability at Scale: Cluster Upgrade Endurance Test
...
While this Proof of Concept (PoC) demonstrates functional validity, enterprise deployments should consider the following enhancements for a production-grade rollout:
...
2.  **Security Hardening:** 
    *   Restrict the `privileged` security context where possible by using granular Linux capabilities (e.g., `CAP_NET_BIND_SERVICE`) instead of full privilege.
    *   Implement host-level `iptables` or `nftables` rules to whitelist access to port `18898` exclusively from the Load Balancer's IP addresses.
3.  **Observability:** Instrument the agent to export Prometheus metrics (e.g., `router_health_status`, `check_latency_ms`), allowing platform teams to monitor the translator's performance and the underlying router's stability in real-time.

**Disclaimer:** This documentation describes a custom implementation pattern. For fully supported, SLA-backed architecture designs, we strongly recommend engaging with **Red Hat Consulting** or referring to the official **Red Hat OpenShift Reference Architecture**.
