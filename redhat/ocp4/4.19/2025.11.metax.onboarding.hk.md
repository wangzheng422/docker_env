...
```bash
# as root
dnf install -y byobu htop jq ipmitool nmstate /usr/bin/htpasswd sshpass
...
metadata:
  annotations:
    createdAt: Saturday, 22-Nov-25 01:02:23 UTC
    createdBy: oc-mirror v2
    oc-mirror_version: 4.19.0-202510142112.p2.g137bd5b.assembly.stream.el9-137bd5b
  name: update-service-oc-mirror
spec:
  graphDataImage: mirror.infra-metax.wzhlab.top:8443/openshift/graph-image:latest
  releases: mirror.infra-metax.wzhlab.top:8443/openshift/release-images
  replicas: 2
```
...
metadata:
  annotations:
    createdAt: Saturday, 22-Nov-25 01:02:11 UTC
    createdBy: oc-mirror v2
    oc-mirror_version: 4.19.0-202510142112.p2.g137bd5b.assembly.stream.el9-137bd5b
  name: itms-release-0
spec:
  imageTagMirrors:
  - mirrors:
    - mirror.infra-metax.wzhlab.top:8443/openshift/release-images
...
spec:
  imageDigestMirrors:
  - mirrors:
    - mirror.infra-metax.wzhlab.top:8443/openshift/release
...
MASTER_01_IP=$CIDR_PREFIX.22.41
...
MASTER_01_INTERFACE=ens4
MASTER_02_INTERFACE=ens4
MASTER_03_INTERFACE=ens4
...
MASTER_01_INTERFACE_MAC=52:54:00:65:de:f1
MASTER_02_INTERFACE_MAC=52:54:00:0c:84:10
...
OCP_GW=$CIDR_PREFIX.0.1
OCP_NETMASK=255.255.0.0
OCP_NETMASK_S=16
OCP_DNS=114.114.114.114
...
export REGISTRY_AUTH_FILE=${BASE_DIR}/data/pull-secret.json

openshift-install --dir=${BASE_DIR}/data/install  agent create pxe-files --log-level=debug

```

### 3.4. Provisioning node from rhel9
...
# 把 kernel 和 initrd 传到各个节点上

cd ${BASE_DIR}/data/install/boot-artifacts

scp agent.x86_64-initrd.img root@<host_ip>:/boot/
scp agent.x86_64-vmlinuz root@<host_ip>:/boot/
...
./metax-k8s-images.1.0.0-20251113-626.run push quay.ocp.usuanova.com:8443/metax

# Push the specific pre-compiled driver image to the local registry
bash ./metax-k8s-driver-image.20251113-631-x86_64.run push quay.ocp.usuanova.com:8443/metax
...
--set registry=quay.ocp.usuanova.com:8443/metax \
...
## for 2025.12.12 new k8s operator version
...
# vllm
docker login --username=cr_temp_user --password=eyJpbnN0YW5jZUlkIjoiY3JpLXpxYTIzejI2YTU5M3R3M2QiLCJ0aW1lIjoiMTc2NTgxMjg3NDAwMCIsInR5cGUiOiJzdWIiLCJ1c2VySWQiOiIyMDcwOTQwMTA1NjYzNDE3OTIifQ:25cfe5fc7e66ab14ccb8f96984f45806981c6417 cr.metax-tech.com && docker pull cr.metax-tech.com/public-ai-release/maca/modelzoo.llm.vllm:1.0.0-maca.ai3.2.1.8-torch2.6-py310-centos9-amd64 


# Push the main operator images to the local registry
bash ./metax-k8s-driver-image.3.3.0.4-x86_64.run push quay.ocp.usuanova.com:8443/metax
# cr.metax-tech.com/cloud/driver-image:3.3.0.4-amd64

tar vxf metax-gpu-k8s-package.0.13.2.tar.gz
# metax-k8s-images.0.13.2.run
# metax-operator-0.13.2.tgz
# metax-gpu-extensions-0.13.2.tgz

bash metax-k8s-images.0.13.2.run push quay.ocp.usuanova.com:8443/metax
# cr.metax-tech.com/cloud/gpu-label:0.13.2-amd64
# cr.metax-tech.com/cloud/gpu-device:0.13.2-amd64
# cr.metax-tech.com/cloud/gpu-aware:0.13.2-amd64
# cr.metax-tech.com/cloud/topo-master:0.13.2-amd64
# cr.metax-tech.com/cloud/topo-worker:0.13.2-amd64
# cr.metax-tech.com/cloud/operator-controller:0.13.2-amd64
# cr.metax-tech.com/cloud/container-runtime:0.13.2-amd64
# cr.metax-tech.com/cloud/driver-manager:0.13.2-amd64

podman pull quay.io/wangzheng422/metax/driver-image:3.3.0.4-amd64-ocp-v02

podman tag quay.io/wangzheng422/metax/driver-image:3.3.0.4-amd64-ocp-v02 quay.ocp.usuanova.com:8443/metax/driver-image:3.3.0.4-amd64-ocp-v02
podman push quay.ocp.usuanova.com:8443/metax/driver-image:3.3.0.4-amd64-ocp-v02


podman image load -i maca-native-3.3.0.4-centos9-amd64.container.xz
# docker.io/library/maca-native:3.3.0.4-centos9-amd64

podman tag docker.io/library/maca-native:3.3.0.4-centos9-amd64 quay.ocp.usuanova.com:8443/metax/maca-native:3.3.0.4-centos9-amd64
podman push quay.ocp.usuanova.com:8443/metax/maca-native:3.3.0.4-centos9-amd64

podman tag cr.metax-tech.com/public-ai-release/maca/modelzoo.llm.vllm:1.0.0-maca.ai3.2.1.8-torch2.6-py310-centos9-amd64 quay.ocp.usuanova.com:8443/metax/maca/modelzoo.llm.vllm:1.0.0-maca.ai3.2.1.8-torch2.6-py310-centos9-amd64
podman push quay.ocp.usuanova.com:8443/metax/maca/modelzoo.llm.vllm:1.0.0-maca.ai3.2.1.8-torch2.6-py310-centos9-amd64



tar vxf metax-operator-0.13.2.tgz

# on each metax gpu node
# create this dummy folder
...
helm uninstall metax-operator -n metax-operator --wait

# create sa for service account metax-operator/metax-gpu-label, and give it priviledges
...
oc label node worker1.ocp.usuanova.com metax-tech.com/maca.ready=true metax-tech.com/runtime.ready=true
oc label node worker2.ocp.usuanova.com metax-tech.com/maca.ready=true metax-tech.com/runtime.ready=true

oc label node worker1.ocp.usuanova.com metax-tech.com/maca.ready- metax-tech.com/runtime.ready-
oc label node worker2.ocp.usuanova.com metax-tech.com/maca.ready- metax-tech.com/runtime.ready-
...
spec:
  ...
  containers:
  - name: vector-add
    image: quay.ocp.usuanova.com:8443/metax/maca-native:3.3.0.4-centos9-amd64
    command: [
        "bash",
        "-c",
        "cp -r /opt/maca/samples/0_Introduction/vectorAdd /home;
...
data:
  ...
  load-metax.sh: |
    ...
    # 0. 预处理：查找 metax.ko
    # 假设容器内包含不同内核版本的 ko，自动寻找当前内核对应的
    KERNEL_VER=$(uname -r)
    # 这里需要根据你镜像的实际结构调整查找路径，或者使用宿主机挂载的路径
    KO_PATH=$(find /metax -name "metax.ko" | head -n 1)
    
    if [ -z "$KO_PATH" ]; then
        echo "ERROR: metax.ko not found for kernel $KERNEL_VER"
        exit 1
    fi
    echo "Found driver at: $KO_PATH"
    chcon -t modules_object_t "$KO_PATH" || echo "Warning: chcon failed, trying anyway..."
    ...
    # 2. 获取内存注册函数地址
    # 注意：我们读取 /host_proc/kallsyms 以确保获取宿主机内核符号
    PROC_FILE="/host_proc/kallsyms"
    if [ ! -f "$PROC_FILE" ]; then
        PROC_FILE="/proc/kallsyms" # 回退到容器内
    fi
    ...
    if [ -z "$REG_FUNC" ] || [ "$REG_FUNC" == "0x" ]; then
        echo "Error: Could not find ib_register_peer_memory_client symbol."
        exit 1
    fi

    echo "Get ib_register_peer_memory_client addr   $REG_FUNC"
    echo "Get ib_unregister_peer_memory_client addr $UNREG_FUNC"
    ...
    if lsmod | grep -q "metax"; then
        ...
        echo "FAIL: metax module failed to load."
        exit 1
    fi
  ...
  entrypoint.sh: |
    ...
    echo ">>> All tasks done. Sleeping to keep pod alive..."
...
spec:
  ...
  template:
    ...
    spec:
      ...
      # --- 1. InitContainer: 负责搬运驱动 ---
      initContainers:
      - name: driver-extractor
        # 使用你的 BusyBox 驱动镜像
        image: quay.ocp.usuanova.com:8443/metax/driver-image:3.3.0.4-amd64-ocp-v02
      ...
      # --- 2. Main Container: 负责执行逻辑 ---
      containers:
      - name: loader
        # 使用 UBI 8，它有 bash, yum, grep, awk 等完整工具
        ...
        securityContext:
          ...
          capabilities:
            add: ["SYS_MODULE", "SYS_ADMIN", "IPC_LOCK", "NET_ADMIN"]
          seLinuxOptions:
            level: s0
            type: spc_t
          runAsUser: 0
        ...
        volumeMounts:
        # 挂载共享的驱动目录，挂载回 /metax 位置，这样脚本不用改路径
        - name: driver-share
          mountPath: /metax
        # 脚本
        - name: scripts
      ...
      volumes:
      # 用于在 InitContainer 和 Main Container 之间传递文件的临时卷
      - name: driver-share
        emptyDir: {}
      - name: scripts
        configMap:
