# openshift 4.19 UPI in agent way, single node
...
create_lv vgdata poolA lv-lib-containers 500G recreate
...
here we will snapshot the vm, and later, we can restore the cluster easily

Since we are using LVM Thin Provisioning, we can leverage efficient snapshots. The strategy is:
1.  **Snapshot (Backup)**: Create a read-only snapshot of the current volume.
2.  **Restore**: Delete the current "dirty" volume and recreate it as a writable snapshot of the backup. This preserves the backup for future restores.

```bash
# Define the list of VMs to manage (e.g., all demo-01 nodes)
DOMAIN_LIST=$(virsh list --all --name | grep demo-01 | grep -v '^$')
VG_NAME="vgdata"
SNAPSHOT_SUFFIX="fresh-install"

# Helper: Stop all VMs
stop_cluster() {
  for dom in $DOMAIN_LIST; do
    echo "Stopping $dom..."
    virsh destroy $dom 2>/dev/null
  done
}

# Helper: Start all VMs
start_cluster() {
  for dom in $DOMAIN_LIST; do
    echo "Starting $dom..."
    virsh start $dom
...
# Function 1: Create Initial Snapshots
# Call this once after installation is stable
# IMPORTANT: Ensure cluster is stopped before running this!
create_snapshots() {
  for dom in $DOMAIN_LIST; do
    echo "Creating snapshot for $dom..."
    
    LV_NAME="lv-${dom}"
    SNAP_NAME="${LV_NAME}-${SNAPSHOT_SUFFIX}"

    # Check if snapshot already exists
    if lvs ${VG_NAME}/${SNAP_NAME} >/dev/null 2>&1; then
      echo "  Snapshot ${SNAP_NAME} already exists. Skipping."
    else
      # Create a Thin Snapshot (Instant)
...
# Function 2: Restore Cluster
# Call this whenever you want to reset the environment
# IMPORTANT: Ensure cluster is stopped before running this!
restore_cluster() {
  for dom in $DOMAIN_LIST; do
    echo "Restoring $dom..."
    ...
    if lvs ${VG_NAME}/${SNAP_NAME} >/dev/null 2>&1; then
      # Remove the current (dirty) volume
      lvremove -f ${VG_NAME}/${LV_NAME}
      
      # Re-create the volume as a snapshot of the backup
      # This creates a new writable volume based on the 'fresh-install' snapshot
      lvcreate -s -n ${LV_NAME} ${VG_NAME}/${SNAP_NAME}

      # Activate the LV if it is skipped
      lvchange -ay -kn ${VG_NAME}/${LV_NAME}
      
      echo "  Restored volume ${LV_NAME}."
    else
...
# Typical workflow: stop -> snapshot/restore -> start
