...
configure it as L4 switch, which mean using L4 health check on translator port 
...
## run with L4 check translator with ocp upgrade

OK, we finally reach the final test, testing the connection during the openshift upgrade.
...
the cluster finished the upgrade.
...
The method listed here, is DIY solution, you need to get support from redhat GPS, to get better improved solution. The enhancement point including
- convert python into golang for faster translator performance, and lower resource needed.
- firewall on the translator tcp port, so only L4 switch can reach.
