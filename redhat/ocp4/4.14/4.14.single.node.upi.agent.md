...
## add another normal worker node
...
# define boot parameter, and create iso
BOOT_ARG=" ip=$WORKER_02_IP::$OCP_GW:$OCP_NETMASK:$WORKER_02_HOSTNAME:$WORKER_02_INTERFACE:none nameserver=$OCP_DNS coreos.inst.install_dev=${WORKER_02_DISK##*/} coreos.inst.ignition_url=http://$HELP_SERVER:5000/worker.ign"
...
virsh destroy ocp4-demo-gpu-worker02
virsh undefine ocp4-demo-gpu-worker02
...
create_lv rl pool00 lv-gpu-worker02 200G recreate
# create_lv vgdata poolA lvacmhub-worker02-data 500G remove

# wipefs --all --force /dev/nvme0n1

SNO_MEM=64

virt-install --name=ocp4-demo-gpu-worker02 \
  --vcpus=32 \
  --ram=$(($SNO_MEM*1024)) \
  --cpu=host-model \
  --disk path=/dev/rl/lv-gpu-worker02,device=disk,bus=virtio,format=raw \
  --os-variant rhel8.3 \
  --network bridge=br-int,model=virtio,mac=52:54:00:12:A1:05 \
...
spec:
  httpProxy: http://172.21.6.8:18801 
  httpsProxy: http://172.21.6.8:18801 
...
deploy a minio for testing only, not for production. becuase official minio will enable https, it will bring so many trouble into app integration.
...
## setup storage for monitor

- https://docs.openshift.com/container-platform/4.14/monitoring/configuring-the-monitoring-stack.html#configuring_persistent_storage_configuring-the-monitoring-stack
...
# disable insight operator

- https://docs.openshift.com/container-platform/4.14/support/remote_health_monitoring/using-insights-operator.html#disabling-insights-operator-gather_using-insights-operator
...
do
  ...
  do
    ...
    do
      echo "Collecting logs for Namespace: $ns, Pod: $pod, Container: $container"
      # 将日志输出到文件，并在每一行前加上命名空间、Pod和容器名称
      oc logs $pod -c $container -n $ns | sed "s/^/${ns}, ${pod}, ${container}: /" >> "$output_file"
    done
  done
done

echo "Logs have been collected in $output_file"
...
- https://docs.openshift.com/container-platform/4.14/architecture/nvidia-gpu-architecture-overview.html
...
听说4.11那个，是用的dynamic plugin，在4.13以后有不兼容的问题，以后有机会，我们给适配一下。
...
# install the minio certified operator

# following docs to apply patch 
# oc edit deployment minio-operator  -n minio-operator
# no need, the operator configure it automatically

# create route for console, this is optional
# for 9443, and https passthrough

# get the login key
oc minio proxy
# Current JWT to login: <xxxxxxxxxxxx>


# create tenant, by using the operator
cat << EOF > ${BASE_DIR}/data/install/tenant.secret.yaml
apiVersion: v1
kind: Secret
metadata:
...
stringData:
  config.env: |-
    ...
    export MINIO_ROOT_PASSWORD=redhatocp
EOF

oc create --save-config -f ${BASE_DIR}/data/install/tenant.secret.yaml -n demo

```

create tenant,

![](imgs/2024-04-03-20-47-15.png)

also needs to config pool size
```yaml
  pools:
...
![](imgs/2024-04-03-21-32-13.png)

expose service

![](imgs/2024-04-03-20-48-25.png)

then, expose the console service through ocp route, you can access the console

![](imgs/2024-04-03-21-29-34.png)
