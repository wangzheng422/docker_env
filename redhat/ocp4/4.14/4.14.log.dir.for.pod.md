  ...
  template:
    ...
    spec:
      ...
      - name: service-account-token
        secret:
          secretName: pod-info-sa-token-5lnpc
      initContainers:
      - name: write-pod-description
        ...
        command: ['sh', '-c', 'oc get pod $POD_NAME -n $NAMESPACE -o yaml --token=$(cat /var/run/secrets/kubernetes.io/serviceaccount/token)> /mnt/pod-deploy.yaml 2>&1 && oc describe pod $POD_NAME -n $NAMESPACE  > /mnt/pod-describe.txt']
      containers:
      - name: my-app-heap-dump
        ...
        volumeMounts:
        - name: pod-description
          # mountPath: /wzh-log/$(MY_CONTAINER_NAME)
          mountPath: /wzh-log/
EOF
