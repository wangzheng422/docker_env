# openshift 4.20 UPI in agent way, single node
...
# 激活主界面列表视图中的列 (Memory, Disk, Network)
...
### quay pass-through proxy setup

following article below to finish post-install config
- [2022.12.quay.cache.init.md](./2022.12.quay.cache.init.md)
...
# dry-run, without download
oc-mirror --v2 --config ${BASE_DIR}/data/mirror/mirror.yaml --authfile ${BASE_DIR}/data/pull-secret.json file://${BASE_DIR}/data/mirror/  --dry-run
...
NODE_SSH_KEY="$(cat ${BASE_DIR}/.ssh/id_ed25519.pub)"
INSTALL_IMAGE_REGISTRY=mirror.infra.wzhlab.top:8443
...
ImageDigestSources:
- mirrors:
  - ${INSTALL_IMAGE_REGISTRY}/cache_quay_io
  source: quay.io
- mirrors:
  - ${INSTALL_IMAGE_REGISTRY}/cache_registry_redhat_io
  source: registry.redhat.io
- mirrors:
  - ${INSTALL_IMAGE_REGISTRY}/cache_registry_connect_redhat_com
...
bash image.registries.conf.quay.sh mirror.infra.wzhlab.top:8443
...
/bin/cp -f ${BASE_DIR}/data/install/99-worker-container-registries.yaml ${BASE_DIR}/data/install/openshift
/bin/cp -f ${BASE_DIR}/data/install/99-master-container-registries.yaml ${BASE_DIR}/data/install/openshift


tee ${BASE_DIR}/data/install/openshift/99-worker-coreos-pwd.yaml << EOF
---
...
spec:
  config:
    ...
    passwd:
      users:
      - name: core
        passwordHash: $(mkpasswd -m sha-512 redhat)
---
apiVersion: machineconfiguration.openshift.io/v1
kind: MachineConfig
metadata:
...
# 把 crack.txt 的内容转换成 MachineConfig
# worker: 设置 root 密码 + SSH 配置
# master: 设置 root 密码 + SSH 配置 + KUBECONFIG 环境变量
tee ${BASE_DIR}/data/install/openshift/99-worker-master-ssh-config.yaml << EOF
---
apiVersion: machineconfiguration.openshift.io/v1
kind: MachineConfig
metadata:
...
spec:
  config:
    ...
    storage:
      files:
      - contents:
          source: data:text/plain;charset=utf-8;base64,$(echo -e 'PasswordAuthentication yes\nPermitRootLogin yes\nClientAliveInterval 1800' | base64 -w0)
        mode: 0644
        overwrite: true
        path: /etc/ssh/sshd_config.d/20-wzh.conf
---
apiVersion: machineconfiguration.openshift.io/v1
kind: MachineConfig
metadata:
  ...
  config:
    ...
    storage:
      ...
      - contents:
        ...
        path: /etc/ssh/sshd_config.d/20-wzh.conf
      - contents:
          source: data:text/plain;charset=utf-8;base64,$(echo -e 'if [ "$(id -u)" = "0" ]; then\n  export KUBECONFIG=/etc/kubernetes/static-pod-resources/kube-apiserver-certs/secrets/node-kubeconfigs/localhost.kubeconfig\n  RET=\`oc config use-context system:admin\`\nfi' | base64 -w0)
        mode: 0644
        overwrite: true
        path: /etc/profile.d/99-wzh-kubeconfig.sh
EOF
...
# 这条命令的意思是：在输出链路，针对 TCP 协议，去往 443 端口的流量，直接丢弃（DROP）
sudo iptables -A OUTPUT -p tcp --dport 443 -j REJECT

# 尝试连接百度（HTTPS 默认就是 443）
curl -I https://www.baidu.com --connect-timeout 5
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-01,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.3 \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-02,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.3 \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-master-03,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.3 \
...
# Take snapshot:
# virsh suspend demo-01-master-01
lvcreate --snapshot --name snap-demo-01-master-01 /dev/vgdata/lv-demo-01-master-01
lvcreate --snapshot --name snap-demo-01-master-02 /dev/vgdata/lv-demo-01-master-02
lvcreate --snapshot --name snap-demo-01-master-03 /dev/vgdata/lv-demo-01-master-03

lvcreate --snapshot --name snap-demo-01-test /dev/vgdata/lv-demo-01-test
# virsh resume demo-01-master-01
...
lvconvert --merge /dev/vgdata/snap-demo-01-master-01
lvconvert --merge /dev/vgdata/snap-demo-01-master-02
lvconvert --merge /dev/vgdata/snap-demo-01-master-03

# lvconvert --merge /dev/vgdata/snap-demo-01-test

# remember to Take snapshot again, after restore.
# virsh suspend demo-01-master-01
lvcreate --snapshot --name snap-demo-01-master-01 /dev/vgdata/lv-demo-01-master-01
lvcreate --snapshot --name snap-demo-01-master-02 /dev/vgdata/lv-demo-01-master-02
lvcreate --snapshot --name snap-demo-01-master-03 /dev/vgdata/lv-demo-01-master-03

virsh start demo-01-master-01
virsh start demo-01-master-02
virsh start demo-01-master-03


# 强制删除快照 (无需确认)
lvremove -y /dev/vgdata/snap-demo-01-master-01
lvremove -y /dev/vgdata/snap-demo-01-master-02
lvremove -y /dev/vgdata/snap-demo-01-master-03
lvremove -y /dev/vgdata/snap-demo-01-test

```

### snapshot again for 4.19
...
# Take snapshot:
# virsh suspend demo-01-master-01
lvcreate --snapshot --name snap-4.19-demo-01-master-01 /dev/vgdata/lv-demo-01-master-01
lvcreate --snapshot --name snap-4.19-demo-01-master-02 /dev/vgdata/lv-demo-01-master-02
lvcreate --snapshot --name snap-4.19-demo-01-master-03 /dev/vgdata/lv-demo-01-master-03
...
lvconvert --merge /dev/vgdata/snap-4.19-demo-01-master-01
lvconvert --merge /dev/vgdata/snap-4.19-demo-01-master-02
lvconvert --merge /dev/vgdata/snap-4.19-demo-01-master-03
...
lvs /dev/vgdata/snap-demo-01-test
  # LV                VG     Attr       LSize  Pool  Origin          Data%  Meta%  Move Log Cpy%Sync Convert
  # snap-demo-01-test vgdata Vwi---tz-k 60.00g poolA lv-demo-01-test

# LVM Thin 卷克隆的终极奥义：根本不需要用 dd 复制！
# 利用 Thin Provisioning 的快照特性，直接针对原快照再打一个快照（或者是直接针对源卷打快照），
# 就能实现"瞬间完成（0秒）"且"底层数据块完全复用，0额外空间膨胀"的完美克隆！
# (如果你之前已经按错误步骤创建了一个空的 lv-demo-01-test-02，请先执行 `lvremove -y /dev/vgdata/lv-demo-01-test-02` 删除它)

lvcreate -s -n lv-demo-01-test-02 /dev/vgdata/snap-demo-01-test

# 因为是从包含 'k' 属性(skip activation)的快照再克隆出来的目标卷，它也会继承 k 属性
# 取消跳过激活的标记(-kn) 并激活这个新克隆的卷 (-ay)
lvchange -kn -ay /dev/vgdata/lv-demo-01-test-02

# 拷贝完成后，为保持环境整洁并防止误操作，建议将其重新取消激活
lvchange -an /dev/vgdata/snap-demo-01-test

cd /data/kvm

virsh dumpxml demo-01-test > new-vm.xml

# 1. 替换虚拟机名称
sed -i 's/<name>demo-01-test<\/name>/<name>demo-01-test-02<\/name>/' new-vm.xml

# 2. 删除老 UUID（定义文件时 libvirt 会自动生成新 UUID）
sed -i '/<uuid>/d' new-vm.xml

# 3. 替换磁盘文件路径 (将 lv-demo-01-test 替换为 lv-new-vm)
sed -i "s|source dev='/dev/vgdata/lv-demo-01-test'|source dev='/dev/vgdata/lv-demo-01-test-02'|g" new-vm.xml

# 4. 删除网卡的 MAC 地址（定义时 libvirt 会自动分配新 MAC，避免两个 VM IP/MAC 冲突）
sed -i '/<mac address=/d' new-vm.xml

virsh define new-vm.xml
# 预期输出: Domain new-vm defined from new-vm.xml

virsh start demo-01-test-02
# 预期输出: Domain new-vm started
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-worker-01,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.6 \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-worker-02,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.6 \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-01-worker-03,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.6 \
...
# simulate air gap
# 1) 允许 88 段访问 192.168.0.0/16 内网
iptables -I FORWARD 1 -i br-ocp -s 192.168.88.0/24 -d 192.168.0.0/16 -j ACCEPT

# 2) 允许 88 段访问 podman 容器网段的 8443
iptables -I FORWARD 2 -i br-ocp -s 192.168.88.0/24 -d 10.88.0.0/16 -p tcp --dport 8443 -j ACCEPT

# 3) 拒绝 88 段去其他任何地方
iptables -I FORWARD 3 -i br-ocp -s 192.168.88.0/24 -j REJECT --reject-with icmp-port-unreachable
...
cat << 'EOF' > ${BASE_DIR}/data/ocp-default.env
CIDR_PREFIX=192.168.88
CIDR_PREFIX_02=192.168.77
# CIDR_PREFIX_02=192.168.77
NTP_SERVER=$CIDR_PREFIX.1
HELP_SERVER=$CIDR_PREFIX.11
# KVM_HOST=192.168.7.11
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-01,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_01_INTERFACE_MAC \
--network bridge=br-ocp,model=virtio,mac=$MASTER_01_INTERFACE_02_MAC \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-02,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_02_INTERFACE_MAC \
--network bridge=br-ocp,model=virtio,mac=$MASTER_02_INTERFACE_02_MAC \
...
--cpu=$SNO_CPU \
--disk path=/dev/vgdata/lv-demo-02-master-03,device=disk,bus=virtio,format=raw,discard=unmap,cache=none,io=io_uring \
--os-variant rhel9.3 \
--network bridge=br-ocp,model=virtio,mac=$MASTER_03_INTERFACE_MAC \
--network bridge=br-ocp,model=virtio,mac=$MASTER_03_INTERFACE_02_MAC \
...
--cpu=host-model \
--disk path=/dev/vgdata/lv-demo-01-test,device=disk,bus=virtio,format=raw,io=io_uring \
--os-variant rhel9.6 \
--network bridge=br-ocp,model=virtio \
--graphics vnc,listen=127.0.0.1,port=59016 --noautoconsole \
--boot menu=on --location /data/kvm/rhel9.iso \
...
access web console:
- http://aws-helper-internal.wzhlab.top:9001/
