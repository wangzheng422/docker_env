...
Since CoreOS nodes do not ship `bpftool`, we use `oc debug` with a custom image that has it pre-installed:

```bash

# Use a privileged debug pod with bpftool available
# (CoreOS minimal image does NOT include bpftool)
IMG="quay.io/wangzheng422/qimgs:centos9-test-2025.12.18.v01"

oc debug node/master-01-demo --image=$IMG -- bash -c '
...
All 16 eBPF programs loaded by `netobserv-ebpf-agent`, mapped to NetObserv features:

```bash

# bpftool prog list | grep netobserv

#  ID  Type             Name                           Feature              xlated    jited  memlock
# ──── ──────────────── ────────────────────────────── ──────────────────── ──────── ─────── ────────
#  133 tracepoint       kfree_skb                      PacketDrop            4208B   2375B   12288B
#  134 kprobe           network_events_monitoring      NetworkEvents         6560B   3695B   16384B
#  135 kprobe           probe_entry_SSL_write          TLS/SSL monitoring      48B     39B    4096B
#  136 sched_cls        tc_egress_flow_parse           Flow (TC egress)      9904B   6148B   20480B
#  137 sched_cls        tc_egress_pca_parse            PacketCapture (TC)      80B     69B   12288B
#  138 sched_cls        tc_ingress_flow_parse          Flow (TC ingress)     9904B   6145B   20480B
#  141 tracing          tcp_rcv_fentry                 FlowRTT (fentry)      2936B   1716B   12288B
#  142 kprobe           tcp_rcv_kprobe                 FlowRTT (kprobe)      2928B   1713B   12288B
#  143 sched_cls        tcx_egress_flow_parse          Flow (TCX egress)     9912B   6151B   20480B
#  145 sched_cls        tcx_ingress_flow_parse         Flow (TCX ingress)    9912B   6148B   20480B
#  146 sched_cls        tcx_ingress_pca_parse          PacketCapture (TCX)     88B     69B   12288B
#  147 kprobe           track_nat_manip_pkt            PacketTranslation     6184B   3733B   16384B
#  148 kprobe           xfrm_input_kprobe              IPSec (input)         3680B   2123B   12288B
#  149 kprobe           xfrm_input_kretprobe           IPSec (input ret)     1312B    876B    4096B
#  150 kprobe           xfrm_output_kprobe             IPSec (output)        3704B   2141B   12288B
#  151 kprobe           xfrm_output_kretprobe          IPSec (output ret)    1336B    894B    4096B
#
# Total eBPF progs on node: 145 (system-wide)
# NetObserv eBPF progs:      16
# Total prog memlock:        208.0 KB  (all NetObserv programs combined)

```

## eBPF maps memory usage

The largest memory consumers are the hash/percpu_hash maps used for flow aggregation and DNS/IPSec tracking:

```bash

# bpftool map list (NetObserv maps only)
...
#
# ★ = large pre-allocated maps (16 MB each)
#
# Total NetObserv map memlock:  ~63.3 MB per node
# Top 3 consumers:
#   dns_flows:         ~16.0 MB  (1M entries hash map)
#   ipsec_ingress_m:   ~16.0 MB  (1M entries hash map)
#   ipsec_egress_ma:   ~16.0 MB  (1M entries hash map)

```

## Network attachment: TCX hooks on every interface

The `tcx_ingress_flow_parse` and `tcx_egress_flow_parse` programs are attached to **every** network interface on the node via TCX (TC eXpress) hooks:
...
# ... (attached to all 37 interfaces on master-01, including pod veth ports)
#
# Total TCX hook points: 37 interfaces × 2 (ingress + egress) = 74 hook points
# All sharing the same 2 prog_ids (143 for egress, 145 for ingress)

```

## Cross-node comparison

All 3 nodes have **identical** NetObserv eBPF program sets:

```bash

# Node comparison (3 × master nodes)
#
# Node             Total eBPF  NetObserv  Prog memlock  Map memlock (est.)
# ──────────────── ────────── ────────── ──────────── ──────────────────
...
#
# Observations:
# - NetObserv programs are IDENTICAL across all nodes (same name, type, bytes_xlated, bytes_jited)
# - Only prog IDs differ (kernel assigns IDs sequentially per node)
# - Total system eBPF count varies (145-209) due to different cgroup_device counts per pod
# - The DaemonSet ensures uniform eBPF instrumentation across the cluster
...
# Feature → eBPF Program → Kernel Hook mapping:
#
# ┌─────────────────────┐
# │ FlowCollector CR    │
# │ features:           │
# │  - FlowRTT          │──→ tcp_rcv_fentry (tracing/fentry on tcp_rcv_established)
# │                     │    tcp_rcv_kprobe (kprobe fallback for older kernels)
# │  - DNSTracking      │──→ tc_*_flow_parse programs parse DNS in the TC path
# │                     │    dns_flows map (16 MB hash, 1M entries)
# │  - PacketDrop       │──→ kfree_skb (tracepoint on skb free = packet drop event)
# │  - PacketTranslation│──→ track_nat_manip_pkt (kprobe on nf_nat_manip_pkt)
# │  - NetworkEvents    │──→ network_events_monitoring (kprobe on network events)
# │  - IPSec            │──→ xfrm_input_kprobe/kretprobe (kprobe on xfrm_input)
# │                     │    xfrm_output_kprobe/kretprobe (kprobe on xfrm_output)
# │                     │    ipsec_ingress_m + ipsec_egress_ma maps (16 MB each)
# │  - UDNMapping       │──→ (no separate program; handled in flow_parse path)
# └─────────────────────┘
#
# TC/TCX Flow Path (attached to every interface):
#   tcx_ingress_flow_parse ←→ tc_ingress_flow_parse (legacy fallback)
#   tcx_egress_flow_parse  ←→ tc_egress_flow_parse  (legacy fallback)
#   tcx_ingress_pca_parse  ←→ tc_egress_pca_parse   (packet capture)
#
# The flow_parse programs are the "main" eBPF programs (~10 KB xlated each)
# that process every packet on every interface. They:
# 1. Extract 5-tuple (src/dst IP, port, protocol)
# 2. Look up/update flow aggregation maps
# 3. Enrich with DNS, RTT, NAT translation data from other maps
# 4. Export via ring buffers when flow expires
...
#
# Note: The IPSec maps (32 MB) are pre-allocated even if IPSec is not
# actively used. Similarly, dns_flows (16 MB) is pre-allocated with
# max_entries=1048576 regardless of actual DNS traffic volume.
# These are the primary memory optimization targets if memory is constrained.
