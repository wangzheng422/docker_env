...
cd /data/ccn
rm -rf /data/ccn/gogs
podman run -d --name gogs-fs --entrypoint "tail" ${LOCAL_REG}/docker.io/wangzheng422/gogs-fs -f /dev/null
...
podman run -d --name nexus-fs --entrypoint "tail" ${LOCAL_REG}/docker.io/wangzheng422/nexus-fs -f /dev/null
