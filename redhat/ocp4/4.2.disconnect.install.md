...
# operator 镜像准备，详细请参考 https://github.com/wangzheng422/docker_env/blob/master/redhat/ocp4/4.2.disconnect.operator.md
bash operator.sh
...
# 上面的脚本有一个输出， mirror-image.yaml ，是重要的输出物。

# podman image load -i custom-registry.tgz
podman image push registry.redhat.ren/ocp-operator/custom-registry
...
# 以下命令会造成对api server的压力，有些文件不能成功，失败的yaml重新运行一下就可以
find ./operator/yaml/ -type f -exec oc apply -f {} \;

find ./operator/yaml/ -type f -exec oc delete -f {} \;
...
spec:
  ...
  sourceType: grpc
  image: registry.redhat.ren/ocp-operator/custom-registry:latest
EOF
oc create -f my-operator-catalog.yaml

# 想删除这个离线operator hub，就这样简单的做。
