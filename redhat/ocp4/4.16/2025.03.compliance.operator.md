...
The logic of this guide is as follows (AI generated):
...
graph TD
    ...
    subgraph "Runtime Components"
        CS[ComplianceSuite]
        CScan[ComplianceScan]
        CCR[ComplianceCheckResult]
        CR[ComplianceRemediation]
    end
    ...
    %% Workflow
    User[User] --> SSB
    SSB --> |"Triggers"| Scan[Scan Process]
    Scan --> |"Generates"| Results[Scan Results]
    Results --> |"Viewed as"| Report[HTML Report]
    ...
    class CO operator;
    class PB,P,R,V,SS,SSB config;
    class CS,CScan,CCR,CR runtime;
    class P1,P2,P3,P4,PB1,PB2,SS1,SS2 examples;
    class User,Scan,Results,Report,Cluster user;
```
...
We already have the necessary configuration in place. Let's proceed with running a scan. To run a new scan, just define a new `scan setting binding`. Here is an example of how to do it, it will create a scan with 2 profiles.
...
Wait a moment, the results are created in the format of CRs.

<img src="imgs/2025.03.compliance.operator.md/2025-03-12-11-12-04.png" height="400px">

Let's filter out some kind of information, and check it in detail.
...
Checking the compliance result using CRs is inconvenient, the offical document shows a practical way to generate a human-readable report.
