...
We needs some pre-installed plugins related with azure sso and ado, following the [offical docs](https://docs.redhat.com/en/documentation/red_hat_developer_hub/1.3/html/dynamic_plugins_reference/con-preinstalled-dynamic-plugins).
...
global:
  ...
  dynamic:
    plugins:
      ...
      # Azure UI
      # redhat version is too old, has some compatibility issue with azure devops
...
After you path the helm config, the rhdh will deploy a new version, but it will not process, because the new config depends on a configmap, which is not existed right now. We will craete it in the next step.
...
--from-literal=AZURE_DEVOPS_PERSONAL_ACCESS_TOKEN=$AZURE_DEVOPS_PERSONAL_ACCESS_TOKEN \
