...
# INFO Login to the console with user: "kubeadmin", and password: "vtiys-Sxtdd-KQ6rS-T6DIk"
# DEBUG Time elapsed per stage:
# DEBUG          bootstrap: 23s
# DEBUG            masters: 16m33s
# DEBUG Bootstrap Complete: 19m51s
# DEBUG  Bootstrap Destroy: 11s
# DEBUG  Cluster Operators: 8m30s
# INFO Time elapsed: 45m29s
...
/bin/cp -f nfs.all.yaml nfs.all.yaml.bak

sed -i 's/storageClassName: odf-lvm-vg1/storageClassName: hostpath-csi/' nfs.all.yaml
sed -i 's/storage: 5Gi/storage: 450Gi/' nfs.all.yaml

oc create --save-config -n nfs-system -f nfs.all.yaml
...
# hostpath-csi   kubevirt.io.hostpath-provisioner   Delete          Immediate           false                  21h

virtctl image-upload dv openstack-base-img -n openstack --size=50Gi --image-path=/data/swap/rhel-8.6-x86_64-kvm-wzh.qcow2  --storage-class redhat-ren-nfs --insecure
...
# overcloud   1                               true           Provisioning   One or more OSVMSets are provisioning

oc get openstackvmsets -n openstack
# NAME         DESIRED   READY   STATUS         REASON
# controller   3                 Provisioning   Provisioning of VirtualMachines in progress

oc get virtualmachines
# NAME           AGE     STATUS    READY
# controller-0   5m31s   Stopped   False
# controller-1   5m31s   Stopped   False
# controller-2   5m31s   Stopped   False
