...
# back to 101
cd /data/kvm/
for i in ocp4-ipi-osp-master-0{1..2}
do
  echo -ne "${i}\t" ; 
...
mkdir -p /etc/crts
scp root@192.168.77.11:/etc/crts/* /etc/crts/
...
VAR_COREOS_VERSION=`openshift-install coreos print-stream-json | jq .architectures.aarch64.artifacts.metal.release -r`
...
VAR_URL=`oc get infraenv/${ACM_DEMO_CLUSTER} -n ${ACM_DEMO_CLUSTER} -o json | jq -r .status.bootArtifacts.rootfs`
wget --no-check-certificate -O ocp-bf2-aarch64-rootfs.img $VAR_URL

sudo /bin/cp -f ocp-bf2-aarch64-rootfs.img /data/ocp4/
...
# scp root@192.168.77.11:/home/3nodeipi/data/install/ocp-bf2-aarch64-rootfs.img  ./
...
spec:
  release:
    image: quay.io/wangzheng422/ocp-release:4.11.21
  pullSecret:
...
![](imgs/2023-02-27-16-54-36.png)

```bash
# copy the command, and run on bf2
# login into bf2
ssh core@192.168.77.55
