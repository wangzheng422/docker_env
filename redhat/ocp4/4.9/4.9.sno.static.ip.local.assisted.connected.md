...
# 部署assist install service

assist install service有2个版本，一个是cloud.redhat.com上面那个，同时还有一个本地版本，两个版本功能一样，因为我们需要有定制需求，所以我们选择本地版本。
...
# 定制assist install service的配置

默认的assist install service创建的iso，要去实验网络必须有dhcp服务，我们要做的是static ip，那么我们就要定制一下assist install service, 激活他现在还是隐藏的功能（暂时没有官方支持）。
...
# 向assist install service发送请求，进行定制
...
# 在assist install service里面配置sno参数

回到assist install service webUI，能看到node已经被发现
