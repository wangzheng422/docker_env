...
内部交流材料（需内网访问）
- [交流PPT](https://docs.google.com/presentation/d/11_D8lJhJwv5r5GG1Kb9Bjg7jtTO00QvwF1WugBzEXxY/edit?usp=sharing)
- [交流录像](https://drive.google.com/file/d/1mTRQCsOe10Teeh5HWIsOV417ZAzDVk83/view?usp=sharing)
...
spec:
  machineConfigSelector:
    matchExpressions:
      - {key: machineconfiguration.openshift.io/role, operator: In, values: [worker,worker-rt-2]}
  nodeSelector:
    matchLabels:
...
oc label node worker-0.ocp4.redhat.ren node-role.kubernetes.io/worker-rt=""
...
spec:
  ...
  machineConfigPoolSelector:
    machineconfiguration.openshift.io/role: worker-rt-2
EOF
oc create -f /data/install/performance-2.yaml

# oc delete -f /data/install/performance-2.yaml

# oc apply -f /data/install/performance.yaml
...
worker-2 enable hyper-thread, to test l1app crash
...
# performance addon operator, try disable selinux
...
# https://www.redhat.com/sysadmin/manipulate-process-priority#fromHistory
