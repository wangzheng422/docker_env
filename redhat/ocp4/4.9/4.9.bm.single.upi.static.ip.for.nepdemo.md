# openshift 4.9 静态IP 半离线 baremetal 安装
...
打包好的安装包，在这里下载，百度盘下载链接:
- 4.9.5
  - 链接: https://pan.baidu.com/s/1VZmavrhnA9KYj92d8hVpdQ 提取码: qf5i 
  - (redhat internal) https://drive.google.com/drive/folders/1pkknqPamIdGLSCDYH69BDVxpUXRboZcs?usp=sharing
- 4.9.12
...
准备离线安装介质的文档，已经转移到了这里：[4.9.build.dist.md](4.9.build.dist.md)
...
```bash
# boot host using live.iso
# on console
nmcli con mod 'Wired connection 2' ipv4.method manual ipv4.addresses 192.168.7.18/24
nmcon con up 'Wired connection 2'

# on helper, write a script to store network setip and core-install script
mkdir -p /var/www/html/script/
cat << EOF > /var/www/html/script/worker-2.sh
nmcli con modify 'Wired connection 1' con-name eno1
nmcli con modify 'Wired connection 2' con-name eno2
nmcli con modify eno1 ipv4.method disabled ipv6.method disabled autoconnect no
nmcli con up eno1
nmcli con modify eno2 ipv4.method manual ipv6.method disabled ipv4.addresses 192.168.7.18/24 ipv4.gateway 192.168.7.11 ipv4.dns 192.168.7.11 autoconnect yes
nmcli con up eno2
...
# on host, after boot live.iso, run command in console
sudo -i

nmcli con mod 'Wired connection 2' ipv4.addresses 192.168.7.18/24
nmcli con up 'Wired connection 2'

curl -o install.sh http://192.168.7.11:8080/script/worker-2.sh
bash install.sh
