...
- [x] 支持实时操作系统，并支持自定义kernel参数
  - [x] 配置过程和成果，在[单独的文章中](/redhat/ocp4/4.9/4.9.real-time.kernel.baicell.md)
- [x] 支持单节点openshift，并支持实时操作系统
  - [x] 单节点openshift离线部署，在[单独的文章中](/redhat/ocp4/4.9/4.9.sno.for.baicell.md)
  - [x] 单节点激活实时操作系统，在[单独的文章中](/redhat/ocp4/4.9/4.9.real-time.kernel.baicell.sno.md)
  - [x] 单节点实时操作系统，激活hyper thread，在[单独的文章中](/redhat/ocp4/4.9/4.9.real-time.kernel.baicell.sno.md#performance-addon-operator-for-ht)
- [x] license使用容器化的方式注入
- [x] service 用 host port + ocp router的方式暴露
- [x] 容器启动后，自动加载基站进程
  - [x] 使用systemd的方式启动
  - [x] inject env into systemd
- [x] fpga driver使用容器化方式加载
  - [x] check and set second ip address
- [x] 基础镜像梳理
  - [ ] 是否支持 rhel8/centos8 ? 
- [x] helm chart/operator方式部署
  - [x] 本文中记录了开发过程，代码和成果在[单独的项目中](https://github.com/wangzheng422/baicell-helm-operator)。
- [ ] CI/CD
  - [x] demo, [成果在单独的文章中](/redhat/ocp4/4.9/4.9.ci.cd.demo.md)
  - [ ] 实施
- [ ] selinux
  - [x] [selinux for fpga driver](#selinux)
  - [ ] non-root in container
- [ ] support replication
  - [ ] start ran app using parameter
  - [ ] disaster recovery
  - [ ] HA solution?
  - [ ] monitoring and 运维
- [ ] PTP
- [ ] network solution
  - [ ] 2 nic ?
- [ ] issues
  - [ ] oc rsh break after some times
  - [ ] lspci show muliple domain number, blocks the fpga firmware upgrade
  - [ ] oam through route

## 分析

前2个需求，可以做成一个，容器启动自动加载，用systemd，改一版镜像。最后面的那个加载fpga driver的，单独做一个

# license, serivce, route

## configmap way
是用config map的方式，向容器注入license.
```bash
...
## host path for single-node openshift
用本机host path的方式，挂载到容器，从而注入license.
```bash
...
## host path for 3-node cluster
这里描述一下，3节点openshift，怎么挂在本地目录。
```bash
# 创建host path
cat << EOF > /data/install/host-path.yaml
---
...
kind: MachineConfig
metadata:
  name: 50-set-selinux-for-hostpath-baicell-worker-rt
  labels:
...
cat << EOF > /data/install/hostpath-demo.yaml
---
apiVersion: apps/v1
kind: Deployment
metadata:
          ...
          requiredDuringSchedulingIgnoredDuringExecution:
            ...
            - matchExpressions:
              - key: kubernetes.io/hostname
                ...
                - worker-0
EOF
oc create -f /data/install/hostpath-demo.yaml

oc delete -f /data/install/hostpath-demo.yaml
...
buildah bud -t registry.ocp4.redhat.ren:5443/baicell/fpga-driver:v04 -f fpga.dockerfile .

buildah push registry.ocp4.redhat.ren:5443/baicell/fpga-driver:v04
...
spec:
  ...
  template:
    ...
    spec:
      ...
      initContainers:
      - name: copy
        image: registry.ocp4.redhat.ren:5443/baicell/fpga-driver:v04
...
## check and set second ip address
...
cat << EOF > set_ip.dockerfile
FROM registry.redhat.io/rhel8/support-tools:8.4
...
EOF
buildah bud -t quay.io/baicell/fpga-driver:set_ip.v06 -f set_ip.dockerfile .

buildah push quay.io/baicell/fpga-driver:set_ip.v06
...
# https://unix.stackexchange.com/questions/487786/selinux-is-blocking-my-external-kernel-module#fromHistory
...
buildah bud -t registry.ocp4.redhat.ren:5443/baicell/fpga-driver:v07 -f fpga.dockerfile .

buildah push registry.ocp4.redhat.ren:5443/baicell/fpga-driver:v07
...
spec:
  ...
  template:
    ...
    spec:
      ...
      initContainers:
      - name: copy
        image: registry.ocp4.redhat.ren:5443/baicell/fpga-driver:v07
        command: ["/bin/sh", "-c", "tar zvxf /BaiBBU_DXSS_1.0.18--4.18.0-305.19.1.PKG --strip 1 -C /baicell/driver/ && /bin/cp -f /ocp4.install.sh /baicell/driver/ "]
        imagePullPolicy: Always
        volumeMounts:
        - name: driver-files
...
# for inject parameters file into app

```bash
oc create configmap du.data.model.for.baicell  \
    --from-file=file1=./TR196_gNodeB_CU_Data_Model.xml
...
cat << EOF > /data/install/fpga.sa.yaml
---
apiVersion: v1
kind: ServiceAccount
metadata:
...
oc create -f /data/install/fpga.sa.yaml

# to restore
oc delete -f /data/install/fpga.sa.yaml
