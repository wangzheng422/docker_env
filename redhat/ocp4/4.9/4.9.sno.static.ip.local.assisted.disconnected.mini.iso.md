...
![](./dia/4.9.ais.disconnected.static.ip.drawio.svg)
...
定制的要点如下：
1. iso启动盘的kargs加上静态ip地址，并改变rootfs 下载地址
2. 由于coreos安装的时候要至少启动2次，每次的启动以后，静态ip的配置方法都不一样，所以要用2中方式，写静态ip的配置文件。
3. 一个是第一次启动，需要的/etc/NetworkManager/system-connections/${SNO_IF}.nmconnection
4. 一个是第二次启动，需要的nmstat配置
5. 还需要把内网的镜像仓库证书给写进去
6. 还需要把内网的离线镜像库的配置写进去
...
PULL_SECRET='{"auths":{"registry.ocp4.redhat.ren:5443": {"auth": "ZHVtbXk6ZHVtbXk=","email": "noemail@localhost"},"'${INSTALL_IMAGE_REGISTRY}'": {"auth": "'$( echo -n 'quayadmin:password' | openssl base64 )'","email": "noemail@localhost"}}}'

echo $PULL_SECRET

CLUSTER_ID=bd2c8765-73b8-44cf-b908-02e5603271b1
...
# e6238099-9927-4925-83e0-c3a198f785fd
# db4449a5-d5f9-47b5-9d72-fafac5e96fbb
...
- mirrors:
  ...
  - ${INSTALL_IMAGE_REGISTRY}/ocp4/release
...
用户名密码是：  kubeadmin  /   3VUQK-4NDwI-YuKg8-KgmRY
