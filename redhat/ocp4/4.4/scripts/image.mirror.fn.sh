...
mirror_image() {
    ...
    if [[ $var_skip == 0 ]]; then
        ...
        if oc image mirror --filter-by-os='linux/amd64' $docker_image $local_image_url; then
...
add_image_file() {
    ...
    if [[ $var_skip == 0 ]]; then
        # if skopeo copy "docker://"$docker_image "docker://"$local_image_url; then
        if oc image mirror --filter-by-os='linux/amd64' -a /data/pull-secret.json --dir=${MIRROR_DIR}/oci/ $docker_image file://$local_image_url ; then
            echo -e "${docker_image}\t${local_image_url}" >> pull.add.image.ok.list
            # echo -e "${yaml_image}\t${yaml_local_image}" >> yaml.add.image.ok.list
            # echo -e "${domain_part}" >> yaml.add.image.ok.list
        else
...
add_image_load_oci_file() {
    ...
    if [[ $var_skip == 0 ]]; then
        # if skopeo copy "docker://"$docker_image "docker://"$local_image_url; then
        if oc image mirror --filter-by-os='linux/amd64' --from-dir=${MIRROR_DIR}/oci/ file:/${file_name} $local_image_url ; then
            echo -e "${docker_image}\t${local_image_url}" >> pull.add.image.ok.list
            # echo -e "${yaml_image}\t${yaml_local_image}" >> yaml.add.image.ok.list
            # echo -e "${domain_part}" >> yaml.add.image.ok.list
        else
