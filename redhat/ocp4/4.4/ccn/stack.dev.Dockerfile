...
RUN cd /tmp && git clone https://github.com/wangzheng422/cloud-native-workshop-v2-labs-solutions && cd cloud-native-workshop-v2-labs-solutions && git checkout master && cd m4  && for proj in order-service  ; do mvn -fn quarkus:add-extension -Dextensions="resteasy-jsonb,mongodb-client" -f ./$proj  ; mvn -fn  clean package -DskipTests -f ./$proj ;  done 
