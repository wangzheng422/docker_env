...
RUN cd /tmp && git clone https://github.com/RedHat-Middleware-Workshops/cloud-native-workshop-v2-labs-solutions && cd cloud-native-workshop-v2-labs-solutions && git checkout master && for proj in m1/catalog m1/inventory m1/monolith m2/catalog m2/inventory m2/monitoring m2/monolith m3/catalog m3/inventory m4/*-service ; do mvn -f ./$proj dependency:resolve-plugins dependency:resolve dependency:go-offline clean compile -DskipTests ; mvn -f ./$proj  clean package  ; mvn clean spring-boot:repackage -f ./$proj ; mvn clean   spring-boot:start -f ./$proj ; mvn clean spring-boot:stop -f ./$proj ; done && cd /tmp && rm -rf /tmp/cloud-native-workshop-v2-labs-solutions
