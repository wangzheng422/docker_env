# openshift 4.6 静态IP离线 baremetal 安装，采用边缘部署模式，3节点集群
...
ocp_client: "file:///data/ocp4/4.6.12/openshift-client-linux-4.6.12.tar.gz"
ocp_installer: "file:///data/ocp4/4.6.12/openshift-install-linux-4.6.12.tar.gz"
ppc64le: false
arch: 'x86_64'
chronyconfig:
  enabled: true
...
virt-install --name=ocp4-master0 --vcpus=8 --ram=49152 \
--cpu=host-model \
...
virt-install --name=ocp4-master1 --vcpus=8 --ram=49152 \
--cpu=host-model \
...
virt-install --name=ocp4-master2 --vcpus=8 --ram=49152 \
--cpu=host-model \
...
# INFO Login to the console with user: "kubeadmin", and password: "sZqKe-MQ7m8-7z8EU-sXSAf"
```
