...
add_image_file() {
    ...
    if [[ $var_skip == 0 ]]; then
        ...
        if oc image mirror --filter-by-os=linux/amd64 --keep-manifest-list=false -a /data/pull-secret.json --dir=${MIRROR_DIR}/oci/ $docker_image file://$local_image_url ; then
