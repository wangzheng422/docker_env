...
wanted_operator_list=$(cat redhat-operator-image.list | awk '{print $2}' \
  | sed 's/\..*//g' | sort | uniq
)
...
build_image_list() {
  ...
  VAR_FINAL=`cat $VAR_INPUT_FILE | grep $VAR_OPERATOR | awk '{print $2}'   | sort | uniq | tail -1`

  echo $VAR_FINAL

  cat $VAR_INPUT_FILE | grep $VAR_FINAL | awk '{print $1}' >> $VAR_OUTPUT_FILE
}
...
bash add.image.sh mapping-redhat.list 

/bin/cp -f pull.add.image.failed.list pull.add.image.failed.list.bak
bash add.image.resume.sh pull.add.image.failed.list.bak
