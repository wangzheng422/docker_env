...
openssl req -x509 -new -nodes -key ca.key -subj "/CN=*.ocp4.redhat.ren" -days 365000 -out ca.crt
openssl req -new -sha256 \
    -key ca.key \
    -subj "/C=CN/ST=Beijing/L=Beijing/O=RedHatRen/OU=Devops/CN=*.ocp4.redhat.ren" \
    -reqexts SAN \
...
openssl x509 -req -days 365000 \
    ...
    -extfile <(printf "subjectAltName=DNS:registry.ocp4.redhat.ren,DNS:*.ocp4.redhat.ren,DNS:*.redhat.ren") \
    -out redhat.ren.crt
...
$( cat /data/ocp4/redhat.ren.crt | sed 's/^/   /g' )
imageContentSources:
- mirrors:
