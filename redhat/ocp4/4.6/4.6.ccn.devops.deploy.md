...
cd /data/ccn
podman create --name swap docker.io/wangzheng422/gogs-fs:gitea-2020-12-26-1325 ls
...
cd /data/ccn/
podman create --name swap docker.io/wangzheng422/nexus-fs:maven-2020-12-25-2024 ls
...
podman run -d -p 9001:9001 -it --name etherpad -v /data/ccn/etherpad:/opt/etherpad-lite/var:z docker.io/etherpad/etherpad:latest

# deploy mta vscode extenstion to helper web server
mkdir -p /data/ccn/vscode
mkdir -p /var/www/html/ccn/
cd /data/ccn/vscode
podman create --name swap docker.io/wangzheng422/imgs:mta-vscode-extension.vsix-2020-12-30-1012 ls
...
mkdir -p /data/pip3
cd /data/pip3
podman create --name swap docker.io/wangzheng422/base-fs:pip3-whl-2020-07-05 ls
...
# in module 4
cat << EOF >> ./.npmrc
registry = http://nexus.ocp4.redhat.ren:8081/repository/npm-all/
EOF

cat << EOF >> ./.bowerrc
{
    "registry" : {
      "search" : [ "http://nexus.ocp4.redhat.ren:8081/repository/bower-all/" ]
     },
