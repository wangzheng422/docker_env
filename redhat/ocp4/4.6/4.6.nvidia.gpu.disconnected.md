# nvidia gpu for openshift 4.6 disconnected

## 简介

本次实验是openshift编译GPU场景的一部分，主要关注于nvidia gpu如何在离线的情况下安装。关于如何gpu passthrough到kvm，模拟边缘gpu主机，见[这个文档](./4.6.disconnect.bm.upi.static.ip.on.rhel8.edge.md)

以下是讲解视频

[<kbd><img src="imgs/2021-01-21-16-10-17.png" width="600"></kbd>]()
...
# 注意，这里需要的包，需要先部署以下gpu operator，然后看看driver的日志，里面装什么包，这里替换成相应的包，不用版本的gpu operator要求不同，所以这里的包也不一样。
...
## nvidia driver image
默认 nvidia gpu driver pod 是需要联网下载各种包的，这里面还设计到订阅，非常麻烦，而且离线无法使用。

我们刚才已经做了一个离线的repo仓库，那么我们就需要定制以下driver image，让他直接用离线的repo仓库就好了。
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: demo1
          image: nvcr.io/nvidia/tensorrt:20.12-py3
