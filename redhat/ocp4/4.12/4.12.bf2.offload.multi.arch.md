...
oc adm release mirror -a $SEC_FILE --keep-manifest-list=true \
  --from=quay.io/wangzheng422/ocp-release:$BUILDNUMBER \
  --to=quaylab.infra.wzhlab.top:5443/ocp4/openshift4


oc adm release mirror -a $SEC_FILE --keep-manifest-list=true \
  --from=quay.io/openshift-release-dev/ocp-release:4.12.9-multi \
  --to=quaylab.infra.wzhlab.top:5443/ocp4/openshift4

oc adm release info -a $SEC_FILE quay.io/openshift-release-dev/ocp-release:4.12.9-multi  --filter-by-os=linux/arm64

oc adm release info -a $SEC_FILE quay.io/openshift-release-dev/ocp-release:4.12.9-multi  --filter-by-os=linux/amd64 --commit-urls

# get the binary
mkdir -p /data/ocp-4.12.9-multi
cd /data/ocp-4.12.9-multi
...
VAR_IMAGE=`oc adm release info -a $SEC_FILE quay.io/openshift-release-dev/ocp-release:4.12.9-multi  --filter-by-os=linux/amd64 --image-for=machine-os-images` --filter-by-os=linux/amd64
...
# oc adm release extract -a $SEC_FILE --tools=true  quay.io/openshift-release-dev/ocp-release:4.12.9-multi  --filter-by-os=linux/arm64

mkdir -p ostree

VAR_IMAGE=`oc adm release info -a $SEC_FILE quay.io/openshift-release-dev/ocp-release:4.12.9-multi  --filter-by-os=linux/arm64 --image-for=machine-os-images`
...
# wget -O openshift-client-linux-${BUILDNUMBER}.tar.gz https://mirror.openshift.com/pub/openshift-v4/aarch64/clients/ocp/${BUILDNUMBER}/openshift-client-linux-${BUILDNUMBER}.tar.gz
# wget -O openshift-install-linux-${BUILDNUMBER}.tar.gz https://mirror.openshift.com/pub/openshift-v4/aarch64/clients/ocp/${BUILDNUMBER}/openshift-install-linux-${BUILDNUMBER}.tar.gz
...
oc adm release mirror -a $SEC_FILE --keep-manifest-list=true \
  --from=quay.io/wangzheng422/ocp-release:$BUILDNUMBER-aarch64 \
  --to=quaylab.infra.wzhlab.top:5443/ocp4/openshift4


# on helper, as root
# get rootfs.img for arm
mount rhcos-live.aarch64.iso /mnt/
/bin/cp -f /mnt/images/pxeboot/rootfs.img  /data/ocp4/ocp-bf2-aarch64-rootfs.img
umount /mnt

# cd /data/ocp4
# export BUILDNUMBER=4.12.9
# wget -O ocp-bf2-aarch64-rootfs.img https://mirror.openshift.com/pub/openshift-v4/aarch64/dependencies/rhcos/${BUILDNUMBER%.*}/latest/rhcos-live-rootfs.aarch64.img
