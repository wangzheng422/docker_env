...
可以看到，keepalived，会在节点上，根据service的定义，创建second IP，然后外部流量，就从这个IP地址，进入集群。这是一种k8s LoadBalancer的实现方式，和ingress controller的方式对比，就是天然支持tcp模式的4层转发。
...
spec:
  ...
  containers:
  - name: php
    image: "quay.io/wangzheng422/php:demo"
---
apiVersion: v1
kind: Pod
metadata:
...
buildah bud -t quay.io/wangzheng422/php:demo -f php.dockerfile .

buildah push quay.io/wangzheng422/php:demo
