...
podman run -it --rm -v /data/rhel8/dnf:/data/dnf:z \
    --mount type=bind,source=/data/rhel8/entitle/$(ls *.pem | sed -n '2p'),target=/etc/pki/entitlement/entitlement.pem  \
    --mount type=bind,source=/data/rhel8/entitle/$(ls *.pem | sed -n '2p'),target=/etc/pki/entitlement/entitlement-key.pem \
...
dnf -y download --resolve --alldeps --enablerepo=rhel-8-for-x86_64-baseos-rpms --releasever=8.3 \
