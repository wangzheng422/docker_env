# upgrade openshift 4.10 based rhcos to rhel 9.1 / 升级 openshift 4.10 基础操作系统到 rhel 9.1
...
- openshift4.10.41 os images
  - 百度分享: https://pan.baidu.com/s/1AjjXsmGb2RT5DMNhntOIvg?pwd=2pb2 

注意，安装过程中，有一个小技巧，在本文中有提到。
...
# you have to manually set some config on each node for the install to continue

# kubelet.conf
cat > ${BASE_DIR}/data/install/crack.txt << 'EOF'

sudo sed -i 's/CSIMigrationAWS: false/CSIMigrationAWS: true/' /etc/kubernetes/kubelet.conf
sudo sed -i 's/CSIMigrationAzureDisk: false/CSIMigrationAzureDisk: true/' /etc/kubernetes/kubelet.conf
sudo sed -i 's/CSIMigrationAzureFile: false/CSIMigrationAzureFile: true/' /etc/kubernetes/kubelet.conf
sudo sed -i 's/CSIMigrationGCE: false/CSIMigrationGCE: true/' /etc/kubernetes/kubelet.conf
sudo sed -i 's/CSIMigrationOpenStack: false/CSIMigrationOpenStack: true/' /etc/kubernetes/kubelet.conf
sudo sed -i 's/CSIMigrationvSphere: false/CSIMigrationvSphere: true/' /etc/kubernetes/kubelet.conf
...
# INFO Login to the console with user: "kubeadmin", and password: "J3NiG-mMH2x-3wkN4-NojqJ"
# DEBUG Time elapsed per stage:
# DEBUG Cluster Operators: 12s
# INFO Time elapsed: 13s
