...
而NTP服务，我们认为网络交换机/路由器可以提供。

这里是这个single node ocp的部署[过程记录](./4.10.sno.installer.embed.registry.md)。
...
我们需要向rhcos直接安装pdns, docker distribution等软件，为什么不能用openshift的容器来提供这些服务呢？这里面有一个crio的bug，简单说，如果主机意外重启，crio会把本地镜像全部作废，然后重新从镜像仓库下载。所以，我们的dns, registry服务就不能用容器来启动，否则如果宿主机暴力重启，dns, registry的容器服务都驱动不了，这个节点的openshift服务就无法启动了。
...
最后，用rpm-ostree向rhcos装rpm，是openshift 4自己就在使用的技术，openshift 4 extension功能，比如real-time kernel extension, 就是通过rpm-ostree向rhcos装了对应的kernel包实现的。
...
重启以后，我们就能看到LayeredPackages了，以后版本的openshift 4 会在集群层面支持 LayeredPackages 功能。目前我们只能直接登录rhcos来手动做安装。
...
我们的配置文件位于/etc下面, 上传的镜像位于/var下面，那么节点重启，集群升级，这些目录会不会被重置呢？目前的实测表面不会，按照文档的说法，/etc下面的内容，在升级的时候会进行合并，/var下面的内容，会保留。
