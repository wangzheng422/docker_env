...
![](./dia/4.10.bm.ipi.static.ip.drawio.svg)
...
ACM_DEMO_MNGED_CLUSTER=acm-demo-managed-cluster
...
create_lv vgdata master0lv

virt-install --name=ocp4-master0 --vcpus=16 --ram=49152 \
--cpu=host-model \
--disk path=/dev/vgdata/master0lv,device=disk,bus=virtio,format=raw \
...
cat << EOF > /data/ocp4/ocp4-upi-helpernode-master/vars-static.yaml
---
ocp_version: 4.10.4
ssh_gen_key: false
staticips: true
...
ansible-playbook -e @vars-static.yaml -e '{ staticips: true, bm_ipi: true }'  tasks/main.yml
...
platform:
  baremetal:
    ...
    bootstrapProvisioningIP: 192.168.7.12
    clusterProvisioningIP: 192.168.7.103
    ...
    clusterOSImage: http://192.168.7.11:8080/install/rhcos-openstack.x86_64.qcow2.gz?sha256=$(zcat /var/www/html/install/rhcos-openstack.x86_64.qcow2.gz | sha256sum  | awk '{print $1}')
    hosts:
      - name: master-0
        role: master
        bmc:
...
SNO_NETMAST=255.255.255.0
SNO_NETMAST_S=24
SNO_HOSTNAME=ocp4s-ais-bootstrap
...
storage:
  files:
    - path: /etc/NetworkManager/system-connections/${SNO_CON}.nmconnection
      overwrite: true
      contents:
        inline: |
          [connection]
...
# 在安装过程中，安装程序回检查master-0节点的hostname是不是localhost，不是的话等待网络配置
...
# INFO Login to the console with user: "kubeadmin", and password: "thoJe-MKRw5-ndeeN-mLbY9"
# DEBUG Time elapsed per stage:
# DEBUG          bootstrap: 2m46s
# DEBUG            masters: 15m16s
# DEBUG Bootstrap Complete: 28m28s
# DEBUG  Bootstrap Destroy: 28s
# DEBUG  Cluster Operators: 9m57s
# INFO Time elapsed: 56m56s
