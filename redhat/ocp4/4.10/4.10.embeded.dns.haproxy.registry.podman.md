...
✅注意，本文所述操作，符合红帽支持政策，具体请看 [Support Exception](https://issues.redhat.com/browse/SUPPORTEX-11008)

✅Note that the operations described in this article are in line with the Red Hat support policy, please see for details [Support Exception](https://issues.redhat.com/browse/SUPPORTEX-11008)
...
⚠️ build the image on a rhel with subscription, otherwise, build will fail.
...
这个步骤，和single node ocp是一样的，只不过需要为 3 master 都执行一遍。需要删除mirror log，否则会导致只mirror 上次遗留的image.

This step is the same as single node ocp, but it needs to be executed for all 3 masters. But, you need to delete the mirroring log, otherwise, it will only mirror the images that left last mirroring.
