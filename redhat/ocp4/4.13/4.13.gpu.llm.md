...
RUN dnf install -y wget vim git iproute iputils && \
    dnf install -y 	https://mirror.stream.centos.org/9-stream/AppStream/x86_64/os/Packages/lftp-4.9.2-4.el9.x86_64.rpm && \
    dnf clean all

RUN mkdir -p /proj/miniconda3 && \
    wget https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh -O /proj/miniconda3/miniconda.sh && \
    bash /proj/miniconda3/miniconda.sh -b -u -p /proj/miniconda3 && \
    /proj/miniconda3/bin/conda init bash


RUN mkdir -p /proj/py_env && \
    bash && \
    conda create -p /proj/py_env/chatchat --yes python=3.10 && \
    conda activate /proj/py_env/chatchat && \
    mkdir -p /proj/git_env && \
    cd /proj/git_env && \
