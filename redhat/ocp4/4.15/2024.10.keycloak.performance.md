...
echo 'JAVA_OPTS="$JAVA_OPTS -Djboss.modules.system.pkgs=org.jboss.byteman,org.jboss.logmanager -Djava.util.logging.manager=org.jboss.logmanager.LogManager -Dorg.jboss.logging.Logger.pluginClass=org.jboss.logging.logmanager.LoggerPluginImpl"' >> /opt/rhsso/bin/standalone.conf
...
/opt/rhsso/bin/standalone.sh -b 0.0.0.0 -Djboss.tx.node.id=$PUBLIC_IP --server-config=standalone-ha.xml -Dwildfly.statistics-enabled=true 
EOF
...
inject another script, and run the init user script on local
