...
buildah run onbuild-container yum -y install deltarpm centos-release-gluster epel-release which iproute bind-utils wget htop bash-completion curl net-tools java-1.8.0-openjdk git iperf3 tcpdump pktgen libhugetlbfs-utils
...
buildah run onbuild-container yum -y update 
buildah run onbuild-container yum -y install libhugetlbfs-utils
buildah run onbuild-container yum -y clean all 
