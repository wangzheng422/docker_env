...
打包好的安装包，在这里下载(internal access only)，目前最新版本是ocp 4.3.3： https://drive.google.com/drive/u/2/folders/1KYTMQnurBYwIz9Qh64_G2gILNiDP6C9e

其中包括如下类型的文件：
- ocp4.tgz  这个文件包含了iso等安装介质，以及各种安装脚本，全部下载的镜像列表等。需要复制到宿主机，以及工具机上去。，目前打包的内容，包括了4.3.1, 4.3.2, 4.3.3 .
...
```
Success
Update image:  registry.redhat.ren:5443/ocp4/openshift4:4.3.3-x86_64
...
spec:
  ...
  sourceType: grpc
  image: docker.io/wangzheng422/operator-catalog:redhat-2020-02-17
  publisher: Red Hat
EOF
...
spec:
  ...
  sourceType: grpc
  image: docker.io/wangzheng422/operator-catalog:certified-2020-02-17
  publisher: Certified
EOF
...
spec:
  ...
  sourceType: grpc
  image: docker.io/wangzheng422/operator-catalog:community-2020-02-17
  publisher: Community
EOF
