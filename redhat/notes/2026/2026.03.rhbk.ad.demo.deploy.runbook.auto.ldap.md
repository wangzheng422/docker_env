...
> **源码目录**：`2026/keycloak-guac-webhook/`（Java SPI + Python 同步服务）
...
> Keycloak SPI 源码在 repo 的 `2026/keycloak-guac-webhook/` 目录。

```bash
scp -r 2026/keycloak-guac-webhook/ root@<server-ip>:~/corp-rhbk-demo/keycloak-spi/
...
> 源码在 `2026/keycloak-guac-webhook/guac-sync/`，详见目录中的 README.md。

```bash
scp -r 2026/keycloak-guac-webhook/guac-sync/ root@<server-ip>:~/corp-rhbk-demo/
...
| 组件 | 路径 | 说明 |
|------|------|------|
| Keycloak SPI（Java） | `2026/keycloak-guac-webhook/` | EventListener SPI，监听 LOGIN 事件，调用 /sync |
| guac-ldap-sync（Python） | `2026/keycloak-guac-webhook/guac-sync/` | Flask 服务，查 LDAP 写 Guacamole DB |
