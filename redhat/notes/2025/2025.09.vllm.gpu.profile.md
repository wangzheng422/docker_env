...
We will launch the vLLM container and send a simple inference request to verify its operation.

reference to [redhat offical document](https://docs.redhat.com/en/documentation/red_hat_ai_inference_server/3.2/html-single/getting_started/index#inference-rhaiis-with-podman-nvidia-cuda_getting-started) for deploy rhaiis/vllm details.
...
Now that we've confirmed the server works, we can restart it with the PyTorch profiler enabled. vLLM provides a convenient way to activate the profiler by setting an environment variable. This will instrument the code to capture detailed execution traces.

```bash
# Run the container again, this time setting the VLLM_TORCH_PROFILER_DIR environment variable.
# This instructs vLLM to activate the profiler and save the resulting trace files to the specified directory inside the container.
...
The `forward` method in `Qwen2Model` iterates through the model's layers, which are instances of `Qwen2DecoderLayer`.
...
Finally, the `forward` method of `Qwen2DecoderLayer` reveals the standard components of a transformer block: layer normalization, self-attention, and a multi-layer perceptron (MLP).

- **Source**: [`vllm/model_executor/models/qwen2.py`](https://github.com/vllm-project/vllm/blob/bf1d46913ba8cc772213472a90ad042970138741/vllm/model_executor/models/qwen2.py#L245)
```python
# ...
    def forward(
        self,
...
### Step 3.2: Generate Nsight Report

Run the custom container. The `ENTRYPOINT` will automatically start profiling.
...
# Stop the container with Ctrl+C. Nsight will automatically save the trace file.
...
### Step 3.3: Analyze the Nsight Report
