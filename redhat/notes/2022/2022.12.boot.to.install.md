...
mkdir -p /data/dnf/rhel8-install/ /mnt/rhel8-install/

mount -o loop,ro -t iso9660 /image_directory/image.iso /mnt/rhel8-install/

cp -r /mnt/rhel8-install/ /data/dnf/rhel8-install/
...
SNO_DISK=/dev/vda
SNO_ROOTFS=inst.repo=http://10.32.5.1/RHEL-8/x86_64/iso-contents-root/
...
menuentry 'coreos' --class fedora --class gnu-linux --class gnu --class os {
    ...
    linux /vmlinuz rd.neednet=1 console=tty0 console=ttyS0 ip=$SNO_IP::$SNO_GW:$SNO_NETMAST:$SNO_HOSTNAME:$SNO_IF:none nameserver=$SNO_DNS inst.repo=$SNO_ROOTFS
    echo  'Loading coreos initrd ...'
    initrd /initrd.img /install.img
}
EOF
...
wget -O alma8.iso http://192.168.7.11:5000/alma8.iso
