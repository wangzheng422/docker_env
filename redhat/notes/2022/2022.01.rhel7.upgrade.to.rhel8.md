...
sed '/rhel-7-server-extras-rpms/,/enabled = 0/s/enabled = 0/enabled = 1/' /etc/yum.repos.d/redhat.repo
...
lvremove -f vgdata/lvrhel7
lvcreate -y -L 120G -n lvrhel7 vgdata
wipefs --all --force /dev/vgdata/lvrhel7
...
# iterm2
# max number of bytes of selection to provide to Service: 100000
...
自动重启，然后重新进行selinux relabel

![](imgs/2022-01-11-19-58-01.png)

自动重启，完成升级，可以看到简单的完成状态信息
