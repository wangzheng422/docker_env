...
nvidia官方有gpu operator，这个operatr的作用，是在容器平台节点的宿主机上，安装gpu内核模块，然后安装容器运行时的plugin。完成这些以后，就可以在容器平台上运行gpu容器应用了。

现在我们就要先从nvidia的gpu operator说起。一步一步看看nvidia是怎么做到的。

我们查看[nvidia gpu operator的官网](https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/latest/index.html)。我们可以看到，里面提到的有效信息是，他说gpu operator里面包含了 NVIDIA driver 和 NVIDIA Container Toolkit（Kubernetes device plugin for GPU）, 其他的是监控部分。

那我们看看这个NVIDIA driver在哪里？经过作者查找，其实是在这里：
- https://github.com/NVIDIA/gpu-driver-container/blob/main/rhel9/nvidia-driver

我们分析上述代码，发现他会根据宿主机的os，现场编译一个driver出来，然后modprob来加载。所以，我们可以说，从nvidia角度说，driver就是内核模块。

那NVIDIA Container Toolkit在哪里呢？经过作者查找，其实是在这里：
- https://github.com/NVIDIA/nvidia-container-toolkit/tree/main
- https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html

我们可以通过多个文档，看出来，这一部分是给容器运行时配置plugin
