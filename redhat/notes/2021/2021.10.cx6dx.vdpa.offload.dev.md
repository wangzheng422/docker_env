...
./mlnxofedinstall --dpdk --ovs-dpdk --upstream-libs --force --distro rhel8.4
...
# install dpdk
dnf install -y mlnx-dpdk mlnx-dpdk-devel numactl-devel openvswitch libnl3-devel openssl-devel zlib-devel libpcap-devel elfutils-libelf-devel 
...
modprobe vfio-pci
dpdk-devbind.py -b vfio-pci ${VAR_TMP_PCI%%.*}.2
dpdk-devbind.py -b vfio-pci ${VAR_TMP_PCI%%.*}.3
...
virt-install --name="${DOMAIN}" --vcpus=2 --ram=16384 \
--virt-type qemu \
...
--boot menu=on --location /data/kvm/Rocky-8.4-x86_64-minimal.iso \
--initrd-inject helper-ks-rocky.cfg --extra-args "inst.ks=file:/helper-ks-rocky.cfg" \
--qemu-commandline='-chardev socket,id=charnet1,path=/tmp/sock-virtio0' \
--qemu-commandline='-netdev vhost-user,chardev=charnet1,queues=16,id=hostnet1' \
--qemu-commandline='-device virtio-net-pci,mq=on,vectors=6,netdev=hostnet1,id=net1,mac=e4:11:c6:d3:45:f2,bus=pci.0,addr=0x6,
   page-per-vq=on,rx_queue_size=1024,tx_queue_size=1024'  --print-xml 1
