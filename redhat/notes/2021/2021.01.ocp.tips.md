...
## odf install shell
https://github.com/academiaonline-org/openshift/blob/master/install/05-odf.sh
...
oc get clusterrole -o json | jq -r '[ .items[] | { rname: .metadata.name, resourceNames:.rules[]?|.resourceNames, resource:.rules[]?|.resources }]' > list

oc get role -o json | jq -r '[ .items[] | { rname: .metadata.name, resourceNames:.rules[]?|.resourceNames, resource:.rules[]?|.resources }]' > list
