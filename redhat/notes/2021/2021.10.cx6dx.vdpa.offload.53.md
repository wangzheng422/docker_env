...
tar zvxf *.tgz
cd /data/down/MLNX_OFED_LINUX-5.3-1.0.5.0-rhel8.3-x86_64
dnf install -y tcl tk kernel-modules-extra python36 make gcc-gfortran tcsh unbound createrepo python36-devel kernel-rpm-macros
# we need to build the ofed for rhel8.4, because it is for rhel8.3 by default
./mlnx_add_kernel_support.sh --mlnx_ofed ./ --make-tgz --distro rhel8.4 --kmp --yes --force --skip-distro-check

/bin/cp -f  /tmp/MLNX_OFED_LINUX-5.3-1.0.5.0-rhel8.4-x86_64-ext.tgz /data/down/
cd /data/down
tar zvxf MLNX_OFED_LINUX-5.3-1.0.5.0-rhel8.4-x86_64-ext.tgz
cd /data/down/MLNX_OFED_LINUX-5.3-1.0.5.0-rhel8.4-x86_64-ext
