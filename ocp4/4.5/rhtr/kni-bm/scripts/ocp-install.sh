#!/bin/bash
export OSP_PROJECT="nw979-project"
export GUID="nw979"
export ORIG_PWD=$(pwd)
...
export PULLSECRET=/home/lab-user/pull-secret.json
export OFFLINE=True
...
setup_services() {
  ...
  sudo cp `pwd`/named/dynamic.opentlc.com.zone /var/named
...
rebuild_resolv() {
  echo "search nw979.dynamic.opentlc.com"|sudo tee /etc/resolv.conf
  echo "search dynamic.opentlc.com"| sudo tee -a /etc/resolv.conf
...
build_bridges() {
  ...
  sudo nmcli connection modify baremetal ipv4.addresses 10.20.0.2/24 ipv4.method manual
...
setup_libvirt() {
  ...
  sudo usermod --append --groups libvirt lab-user
...
build_installer() {
  ...
  export GOPATH=/home/lab-user/go
...
set_disconnected() {
  sudo chown -R lab-user:users /nfs
...
#  export UPSTREAM_REPO="registry.svc.ci.openshift.org/ocp/release:$VERSION"
  export PULLSECRET=/home/lab-user/pull-secret.json
  export LOCAL_REG='provision.nw979.dynamic.opentlc.com:5000'
  export LOCAL_REPO='ocp4/openshift4'

  sudo yum -y install podman httpd httpd-tools
  sudo mkdir -p /nfs/registry/{auth,certs,data}
  sudo openssl req -newkey rsa:4096 -nodes -sha256 -keyout /nfs/registry/certs/domain.key -x509 -days 365 -out /nfs/registry/certs/domain.crt -subj "/C=US/ST=NorthCarolina/L=Raleigh/O=Red Hat/OU=Marketing/CN=provision.nw979.dynamic.opentlc.com"
  sudo cp /nfs/registry/certs/domain.crt $(pwd)/domain.crt
  sudo chown lab-user:users $(pwd)/domain.crt
...
"provision.nw979.dynamic.opentlc.com:5000": {
